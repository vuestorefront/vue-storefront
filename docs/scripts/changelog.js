...
const changes = fileName
  ...
  .map((pullRequest, index) => {
    const template = compile(templates.change, { noEscape: true });
...
const versionLineNumber = changelog.map((el) => el.indexOf(version)).findIndex((el) => el > -1);

const template = compile(templates.version, { noEscape: true });

// Update changelog file
versionLineNumber > -1 ? changelog.splice(versionLineNumber + 2, 0, changes) : changelog.splice(2, 0, template({ version, changes }));
