...
This module contains all the logic, components and store related to checkout operations.

## Components

### CartSummary

This component represents a cart summary using `Microcart` as a mixin

### OrderReview

The review of the current order

**Props**

- `isActive` - requires to be true

**Methods**

- `placeOrder()` - if user is registered, places the current order. Otherwise calls `register` method to register the user;
- `register()` - register a user with the data stored in the module state
