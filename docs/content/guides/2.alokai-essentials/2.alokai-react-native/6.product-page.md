...
You will notice that `Product` type is not defined. In order to access SAPCC package types, we need to install the `@vsf-enterprise/sap-commerce-webservices-sdk` package. Run the following command in the root of your project to install the package:
...
Add the following code at the top of the `components/ProductCard.tsx` file:
