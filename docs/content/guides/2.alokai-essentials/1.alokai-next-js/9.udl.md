...
Next, let's prepare Unified API Extension for the Alokai SDK. Create a new file inside `middleware` directory called `types.ts` and add the following code:
...
This file exports two types: `UnifiedApiExtension` and `UnifiedEndpoints`. This is needed for SDK to automatically generate type safe methods based on Unified `apiMethods` object.
...
In order to configure Alokai SDK with Unified Data Layer, we need to change the SDK configuration. Open `storefront/sdk/sdk.config.ts` file and add the following code:
...
Finally, if you run the application, you will see that both home page and product details page are working as expected. Except for one thing. If you try to add a product to the cart, you will see that we have an issue with the `useCart` hook - nothiing happens when you click the "Add to cart" button.
...
<!-- Update with the URL once the guide is ready -->
<!-- ::grid{:columns="3"}
#section-1
