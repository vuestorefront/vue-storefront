...
So far, we've been using `getSdk` function to access sdk on the server side. We could use it as well on the client side
...
Define sdk context and provider by creating `apps/storefront/sdk/alokai-context.tsx` file with the following content:
...
Create `apps/storefront/providers/providers.tsx` file that will aggregate all the providers. The file should have the following content:
