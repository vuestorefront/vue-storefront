---
title: Install and Configure Vue Storefront Middleware
layout: default
navigation:
  icon: tabler:number-2-small
---

# Install and Configure Vue Storefront Middleware

In the last section, we have installed the Next.js application and created a new Vue Storefront project. In this section, we will install and configure the Vue Storefront Middleware.

The Vue Storefront Middleware is a Node.js application that acts as a bridge between the frontend application and the backend. It is responsible for handling all the API requests and responses, as well as managing the state of the application.

In this guide, we will install the Vue Storefront Middleware and configure it to work with SAP Commerce Cloud.
...
![Installing VSF Middleware](./images/nextjs-starter-3.png)

## Install Vue Storefront SAP Commerce Cloud Integration

So far, we have installed the `middleware` application. But in order for it to run and communicate with SAP Commerce Cloud, we need to install the `@vsf-enterprise/sapcc-api` package. This package is a Vue Storefront integration for SAP Commerce Cloud.
...
## Configure Vue Storefront Middleware
...
![Next.js and Middleware Running](./images/nextjs-starter-4.png)

## Summary

In this section, we have installed and configured the Vue Storefront Middleware. We have installed all the necessary dependencies for the `middleware` application and configured it to work with SAP Commerce Cloud. We have also created a new configuration file and added all the necessary environment variables.

In the next section, we will install and configure the Vue Storefront SDK in the Next.js application. 

::grid{:columns="3"}
#section-1
:card{to="/guides/vue-storefront-next-js/install-sdk" title="Install and Configure Vue Storefront SDK" description="Let's install and configure the Vue Storefront SDK in the Next.js application" icon="tabler:number-3-small"}
::