...
Note that the overrides are made here within the api key. This is important because configurations are merged with the main configuration, so their structure must be the same. In the base configuration, these keys are also nested inside of the `api` object.
...
To enable proper typing, create a new type that will define the configuration switcher config. For this purpose, use the `Config` type from the base configuration of the integration whose configuration you're changing.
...
This way, you can override only selected parts of the `api` key, and TypeScript won't require you to provide all keys that are marked as required.
