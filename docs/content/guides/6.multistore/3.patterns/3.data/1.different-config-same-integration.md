...
Integrations are at the core of many eCommerce or CMS applications, and sometimes you may need different configurations for the same integration in your project. This guide will walk you through how to implement multiple configurations and explain when and why you might need them.
...
### Solution:
You need to create and customize the **middleware** configuration for each store. In this case you need to update configuration of SAPCC integration. This solution is possible thanks to **file-based inheritance**. Here's a breakdown:
1. Create a configuration file for each store.
2. Override specific values (like product catalogs) in each file.

Every store will then have its own configuration, with values specific to its needs and use case as shown in example below:
...
Instead of creating separate configuration files for each store, leverage **environment variables** where it is possible. Different environment settings can dynamically configure the middleware or storefront application for each store.

This will help avoiding duplicated files and makes your solution more modular, clean, and easy to maintain.

Let's change the previous configuration example to use environmental variables:
...
For local development, use `.env` files specific to each store. For example:
...
```
Your application can then load these files accordingly during development.
...
- Use **file-based inheritance** to share the majority of the configuration and override only what needs to change for individual stores.
...
Every instance can have its own environment variables configured for middleware and storefront application. To find out how to provide environment variables please check [Console docs](/console/instance/settings/environment-variables).
