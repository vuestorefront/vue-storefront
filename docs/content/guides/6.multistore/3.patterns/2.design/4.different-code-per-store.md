...
Use this snippet to override the `DecoratedPrice` component for a specific store (`store-a`):

```bash
mkdir -p apps/stores/store-a/storefront-unified-nextjs/components
cp apps/storefront-unified-nextjs/components/decorated-price.tsx apps/stores/store-a/storefront-unified-nextjs/components 

```

#### In Next.js

Here's how to override `DecoratedPrice` for a specific store (`store-a`) in a Next.js project:
