...
Welcome to the store management guide! This comprehensive guide will show you how to efficiently manage stores in your Alokai multistore project. Whether you're creating new stores, reorganizing your store hierarchy, or handling store configurations, you'll find everything you need to work effectively with the Alokai CLI.

**Why This Matters**

Managing stores effectively is crucial when working with multiple brands or regions. The Alokai CLI helps you:
- Create new stores with proper configuration and inheritance
- Keep your codebase organized as you add more stores
- Move stores between different parents without breaking dependencies
- Maintain proper configuration as your project grows
...
3. **Store Type**
   - Choose if store should be deployable or not. Inheritance-only stores are not deployable.

4. **Console Project** (for direct stores only)
   - The project name in Alokai Console
   - If you're not sure, just skip it for now
   - Can be updated later in `alokai.config.json`

5. **Cloud Region** (for direct stores only)
...
For direct stores, the CLI adds a new entry with default settings. All of these settings can be modified at any time:
...
```json
{
  ...
  "stores": {
    "my-brand-us": {
      "deployment": {
        "frontend": "nextjs",         // Which frontend to deploy
...
- Inheritance-only stores are not added to `alokai.config.json` as they're not meant to be deployed
