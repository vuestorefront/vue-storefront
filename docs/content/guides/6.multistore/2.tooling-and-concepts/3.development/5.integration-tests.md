...
This guide explains how to create and run integration tests using Playwright in your Alokai multistore project. Integration tests help ensure that your store's features work correctly across different components and pages.
...
Integration tests in Alokai verify that different parts of your store work together correctly. They:
- Run against composed stores (respecting inheritance)
- Use mocked API responses to test user flows and interactions without relying on external services
- Verify UI components and page behavior
- Are running in the Playwright test runner
...
```ts [apps/stores/fashion-brand/playwright/mocks/recommendations/endpoints/getRecommendations.ts]
...
```ts [apps/stores/fashion-brand/playwright/mocks/recommendations/server.ts]
import type { MockFactoryContext } from '@core';
...
```ts [apps/stores/fashion-brand/playwright/mocks/init.ts]
...
```ts [apps/stores/fashion-brand/playwright/setup/pageObjects/recommendations.page.ts]
...
export class RecommendationsPage extends BasePage {
  ...
  override async prepare() {
    // Add custom preparation logic if needed
    return this;
  }
}
```
...
Playwright Test uses fixtures to establish isolated test environments. Fixtures provide tests with all necessary dependencies and are automatically cleaned up between test runs. Let's register our page object as a fixture:

```ts [apps/stores/fashion-brand/playwright/setup/test.ts]
...
export type TestFixtures = {
  // Existing fixtures for other pages
  cartPage: CartPage;
  homepagePage: HomepagePage;
  // Add our new recommendations page fixture
  recommendationsPage: RecommendationsPage;
};

// Create base test object with fixtures
...
export const test = base.extend<TestFixtures>({
  ...
  recommendationsPage: async ({ dataFactory, db, framework, frontendUrl, page, utils }, use) => {
    ...
    // Use: Provide the fixture to the test
...
```ts [apps/stores/fashion-brand/playwright/tests/recommendations.test.ts]
