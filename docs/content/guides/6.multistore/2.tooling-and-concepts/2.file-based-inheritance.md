...
:::tip
Template stores (`fashion-brand` and `sports-brand`) are used to share code between their child stores, while deployable stores (`us-store` and `eu-store`) are the ones that get deployed and run.
:::
