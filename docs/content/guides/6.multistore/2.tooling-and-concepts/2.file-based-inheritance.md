...
- Choose the store type (template or deployable)
...
Template stores (`fashion-brand` and `sports-brand`) are used to share code between their child stores, while deployable stores (`us-store` and `eu-store`) are the ones that get deployed and run. Template stores can define completely different UI components and styling, customer journey flows, product presentation, and regional adaptations that will be inherited by their deployable children.
:::
