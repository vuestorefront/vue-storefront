...
```ts
// helpers/ssr/setCacheControl.ts
...
7. Update `.gitignore`. Add this line at the bottom of the file:
...
export const { getSdk } = createSdk(options, ({ buildModule, middlewareModule, config, getRequestHeaders }) => ({ // [!code ++]
  unified: buildModule(middlewareModule<UnifiedEndpoints>, {
    ...
    cdnCacheBustingId: config.cdnCacheBustingId, // [!code ++]

    defaultRequestConfig: {
      headers: getRequestHeaders(),
    },
    methodsRequestConfig: config.defaultMethodsRequestConfig.unifiedCommerce.middlewareModule, // [!code ++]
  }),
...
10. (Optionally) Update the starting script for the frontend app
...
11. Update the deployment script
