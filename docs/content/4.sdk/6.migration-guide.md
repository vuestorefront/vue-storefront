...
1. Update version of the `@vue-storefront/next` package to version 3.0.0

2. Update the environment variables

For the development mode update the `.env.` file:
...
For deployed project in the Console, go to the **Settings** -> **Environment variables** -> Section **Storefront Application**. Click on the "Add variable", and in the modal fill the name & value of the environment variable. Repeat for all the variables needed to run the app (basically it's all those in the `.env` file).

3. Update the SDK configuration

SDK now it's initialized separately for server-side & client-side. To avoid code duplication, we create a separate files for SDK Factory Options and SDK Config.
...
Now, lets create a `sdk/config.ts` that will be containing the SDK configuration, that for now was in the `sdk/sdk.config.ts` file. The example below presents a standard configuration:
...
Next, we will update the `sdk/sdk.config.ts`. First, let's rename this file to the `sdk/sdk.server.ts`, and after that update the internals:
...
Next step is about the updating the SDK Provider, edit the `sdk/SdkProvider.ts`:
...
As final step, let's update the `pages/_app.tsx` file:
...
1. Update version of the `@vue-storefront/nuxt` package to version 4.1.0
