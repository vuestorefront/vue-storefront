...
Imagine that you're preparing an extension that would add cache control headers to responses based on the configuration.
  ...
  hooks(req, res) {
    return {
      afterCall({ response, callName }) {
        ...
        if (callName in cacheControlConfig) {
          res.set("Cache-Control", cacheControlConfig[callName]);
        }
...
The mechanism in here is universal and can be used in more than one integration. To avoid code duplication, you can prepare a closure that will create a new extension with a given configuration.
