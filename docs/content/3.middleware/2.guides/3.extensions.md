...
To call the extension endpoint, you should the [middlewareModule](/sdk/getting-strated/middlewareModule).

First, you need to prepare the type definition for the extension methods. It would be necessary to pass this type to the `middlewareModule`. To do it, let's aggregate the extension methods in a single object:
...
Then, let's use `WithoutContext` type helper to prepare the type of the endpoints created by the extension methods. As a good practice, it's recommended to use a separate file for the types used in the middleware configuration:
...
Finally, pass the `ExtensionEndpoints` type to the `middlewareModule`:
