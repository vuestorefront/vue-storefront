...
**We provide an ecosystem of independent developer tools** addressing challenges that you will inevitably encounter when building an eCommerce storefront. Each of the tools we provide is focused on solving a specific category of problems. They work best together, but they are independent from each other - each can be easily replaced with a different solution.

## Typical Headless Architecture

Creating an exceptional storefront involves addressing a range of domain-specific challenges that extend well beyond the storefront interface. Factors like infrastructure and the data layer play a crucial role in determining both the speed and reliability of your storefront. Our solutions are designed to assist you in overcoming all of these challenges.

Let’s see what a typical, state-of-the-art headless eCommerce architecture looks like on a high level. You'll soon discover that our product ecosystem is designed to mirror these best practices.
...
On a high level, every third-party vendor integration is similar and contains of two elements:
