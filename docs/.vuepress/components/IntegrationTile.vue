  ...
  >
    ...
    <div class="tile-info">
      <p class="tile-title">
        ...
        <span
          v-if="status !== theme.STATUSES.STABLE"
...
export default {
  ...
  setup(props) {
    ...
    const statusClass = computed(() => {
      const [ key ] = Object
        .entries(theme.value.STATUSES)
        .find(([ _, value ]) => value === props.status);
    ...
    const availabilityClass = computed(() => {
      const [ key ] = Object
        .entries(theme.value.AVAILABILITY)
        .find(([ _, value ]) => value === props.availability);
