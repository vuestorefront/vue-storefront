  ...
  <div class="droppoint-map">
    ...
    <span :key="index" v-for="(field, index) in extraFields">
      ...
      <input
        ...
        @keyup="
          $v.shipping.extraFields[index].$touch();
          setShipping();
        "
      />
      ...
      <span
        class="validation-error"
        v-if="
          $v.shipping.extraFields[index].$error &&
            !$v.shipping.extraFields[index].required
        "
      >
        {{ $t('Field is required') }}
      </span>
  ...
  methods: {
    ...
    getDroppoints() {
      ...
      if (this.searchZipcode) {
        this.loading = true;
        this.error = null;
        let endpoint = this.$config.droppointShipping[this.shippingMethod]
          .endpoint;

        this.$store.dispatch(
          'droppoint-shipping/fetch',
          {
            url:
              endpoint + '/zipcode/' + encodeURIComponent(this.searchZipcode),
            payload: {
              method: 'GET',
