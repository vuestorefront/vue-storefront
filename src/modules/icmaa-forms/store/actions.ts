...
import { formsStorageKey as storageKey } from './'
...
const actions: ActionTree<FormsState, RootState> = {
  single: async (context, options: SingleOptionsInterface): Promise<FormsStateItem> =>
    singleAbstract<FormsStateItem>({ documentType, mutationTypes, storageKey, context, options }),
  list: async (context, options: ListOptionsInterface): Promise<FormsStateItem[]> =>
    listAbstract<FormsStateItem>({ documentType, mutationTypes, storageKey, context, options })
}
