  ...
  async updatePromotedOffers({ commit, rootState }, data) {
    ...
    try {
      const promotedOffersModule = await import(
        /* webpackChunkName: "vsf-promoted-offers-[request]" */ `theme/resource/${promotedBannersResource}.json`
      );
  ...
  async updateHeadImage({ commit, rootState }, data) {
    ...
    try {
      const imageModule = await import(
        /* webpackChunkName: "vsf-head-img-[request]" */ `theme/resource/${mainImageResource}.json`
      );
