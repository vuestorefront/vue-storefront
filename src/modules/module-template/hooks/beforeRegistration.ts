...
export function beforeRegistration({ Vue, config, store, isServer }) {
  if (!isServer)
    console.info(
      'This will be called before extension registration and only on client side'
    );
    ...
    execute: ({ route, store, context }) => {
      return new Promise((resolve, reject) => {
        if (route.name === 'configurable-product') {
          store.state.exampleDataFetchedByLoader =
            'this is just example data fetched by loader on configurable product page';
        } else {
          store.state.exampleDataFetchedByLoader =
            'this is just example data fetched by loader on any page';
        }
        resolve(null);
      });
    }
