...
export default {
  beforeMount () {
    ...
    let inst = this
    let byErinRecommends = builder().query('match', 'erin_recommends', '1').build()
    let byCategoryQuery = builder().query('terms', 'category.category_id', this.product.category.map((cat) => { return cat.category_id })).build()
