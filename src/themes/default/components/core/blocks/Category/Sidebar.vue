<template> 
    <div class="sidebar">
        ...
        <div v-if="filters.size.length">
            <h5>Size</h5>
            <size-button class="size-select mr10" v-for="(size, index) in filters.size" :key="index" :id="size.id" :content="size.label" />
        </div>

        <div v-if="filters.price.length">
            <h5>Price</h5>
            <price-button class="price-select mb10" v-for="(price, index) in filters.price" :key="index" :id="price.id" :content="'$' + price" />
        </div>
    </div>
</template>
