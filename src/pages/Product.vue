  ...
  created () {
    ...
    EventBus.$on('filter-changed', (filterOption) => {
      ...
      self.$store.dispatch('product/configure', { product: self.product, configuration: self.configuration }).then((selectedVariant) => {
        self.$store.dispatch('product/single', { fieldName: 'sku', value: selectedVariant.sku, setCurrentProduct: false, selectDefaultVariant: false }).then((conf_product) => { // TODO: rewrite me, this ruins the cache for offline! add rather option settings for cart item 
          self.configured_product = conf_product
          console.log(conf_product)
        })
      })
    })
  },
