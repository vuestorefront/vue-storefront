...
const getStoredMethods = async ({ customerId, channel }) => {
  try {
    const { data } = await axios.get(
      `${getChannelCtApiUrl()}/merchants/${getChannelPublicKey()}/customers/${customerId}`,
...
const removeStoredMethod = async ({ customerId, paymentInstrumentId, channel }) => {
  try {
    return await axios.delete(
      `${getChannelCtApiUrl()}/merchants/${getChannelPublicKey()}/customers/${customerId}/payment-instruments/${paymentInstrumentId}`,
      apiRequestHeaders(channel)
    );
  } catch (err) {
    console.log(err);
