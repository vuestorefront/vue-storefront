...
context([], 'Checkout - Order Summary', () => {
  ...
  it('Should contain correct data in Order Summary', function () {
    ...
    page.checkout.payment.productRow.each((row, index) => {
      cy.wrap(row).within(() => {
        page.checkout.payment.productTitleSku.contains(data.expectedCartSummary.products[index].name).should('be.visible');
        page.checkout.payment.productTitleSku.contains(data.expectedCartSummary.products[index].sku).should('be.visible');
        page.checkout.payment.productAttributes.contains(data.expectedCartSummary.products[index].size).should('be.visible');
        page.checkout.payment.productAttributes.contains(data.expectedCartSummary.products[index].color).should('be.visible');
        page.checkout.payment.productQuantity.contains(data.expectedCartSummary.products[index].quantity).should('be.visible');
        page.checkout.payment.productPrice.contains(data.expectedCartSummary.products[index].amount).should('be.visible');
      });
    });
  });
