...
export default {
  ...
  setup(props, context) {
    ...
    console.debug({ productReviews, searchReviews });
    ...
    /**
     * SfGalley does not show large image when switching between product variants.
     * The ".sf-image-loaded" class is not applied to the image tag after the image is loaded.
     * The "onload" handler may not be triggered for an image that has been loaded previously.
     */
    ...
    searchReviews({ _limit: 5 });

    onSSR(async () => {
      await search({ slug, customQuery: { products: 'my-products-query' } });
