...
export default {
  ...
  setup(_, context) {
    ...
    const handleBillingAddressSubmit = async billingDetails => {
      ...
      // await loadBillingMethod();
      isSaving.details = false;
    };
    // const handleBillingMethodSubmit = async billingMethod => {
    //   isSaving.method = true;
    //   await saveBillingMethod({ billingMethod });
    //   if (billingMethodError.value.save) {
    //     return;
    //   }
