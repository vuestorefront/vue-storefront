...
describe('[commercetools-composables] useLocale', () => {
  ...
  it('loads values from cookies', async () => {
    (Cookies.get as any).mockImplementation((cookieName) => {
      if (cookieName === cookies.countryCookieName) return 'some-country'
      if (cookieName === cookies.currencyCookieName) return 'some-currency'
      if (cookieName === cookies.localeCookieName) return 'some-locale'
    });
