...
import { CT_COOKIE_NAME, TokenType } from '../types/setup';
...
export const tokenExtension: ApiClientExtension = {
  ...
  extendApp: ({ app, configuration }) => {
    // let currentToken = parseToken(app.request.cookies[CT_COOKIE_NAME]);
    let currentToken = null;
    // if (app.request?.cookies[CT_COOKIE_NAME]) {
    //   currentToken = parseToken(app.request.cookies[CT_COOKIE_NAME]);
    // }
    ...
    function onTokenRemove() {
      ...
      return app.response.clearCookie(CT_COOKIE_NAME);
    }
    ...
    const { tokenProvider } = createSdkHelpers(configuration, TokenType.ServerAccessToken);
    Logger.debug('Successfully generated server access token');

    Logger.debug('Generating quest access token');
    const { tokenProvider: serverTokenProvider } = createSdkHelpers(configuration, TokenType.AnonymousAccesToken);
