...
const mockResult = [{ name: 'extension1', extendApiMethods: { method: () => {} } }];

jest.mock('../../../../src/integrations/helpers/resolveDependency.ts', () => {
  return {
    __esModule: true,
    resolveDependency: (val: string) => mockResult,
  };
});

describe('[lookUpExternal]', () => {
  it('should return array with given extension object', () => {
    const mockExtension = { name: 'extension1', extendApiMethods: { method: () => {} } };
