...
import { CustomQueryFn, UseReview } from '../types';
import { sharedRef } from '../utils';

export declare type UseReviewFactoryParams<REVIEW, REVIEWS_SEARCH_PARAMS, REVIEW_ADD_PARAMS> = {
  searchReviews: (params: REVIEWS_SEARCH_PARAMS, customQuery?: CustomQueryFn) => Promise<REVIEW>;
  addReview: (params: REVIEW_ADD_PARAMS, customQuery?: CustomQueryFn) => Promise<REVIEW>;
};
...
export function useReviewFactory<REVIEW, REVIEWS_SEARCH_PARAMS, REVIEW_ADD_PARAMS>(
  ...
  return function useReview(id: string): UseReview<REVIEW, REVIEWS_SEARCH_PARAMS, REVIEW_ADD_PARAMS> {
    ...
    const search = async (params?: REVIEWS_SEARCH_PARAMS, customQuery?: CustomQueryFn): Promise<void> => {
      try {
        loading.value = true;
    ...
    const addReview = async (params: REVIEW_ADD_PARAMS, customQuery?: CustomQueryFn): Promise<void> => {
      try {
        loading.value = true;
