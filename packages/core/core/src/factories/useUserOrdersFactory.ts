...
import { CustomQueryFn, UseUserOrders } from '../types';
...
export type UseUserOrdersFactoryParams<ORDER, ORDER_SEARCH_PARAMS> = {
  searchOrders: (params: ORDER_SEARCH_PARAMS, customQuery?: CustomQueryFn) => Promise<OrdersSearchResult<ORDER>>;
};
...
export function useUserOrdersFactory<ORDER, ORDER_SEARCH_PARAMS = any>(
  ...
  return function useUserOrders(): UseUserOrders<ORDER> {
    ...
    const searchOrders = async (params?: ORDER_SEARCH_PARAMS, customQuery?: CustomQueryFn): Promise<void> => {
      loading.value = true;
      try {
