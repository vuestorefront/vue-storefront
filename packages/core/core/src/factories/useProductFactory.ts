import { CustomQuery, ProductsSearchParams, UseProduct, BaseFactoryParams } from '../types';
...
export interface UseProductFactoryParams<PRODUCT, PRODUCT_SEARCH_PARAMS extends ProductsSearchParams, API> extends BaseFactoryParams<API> {
  productsSearch: (searchParams: PRODUCT_SEARCH_PARAMS, customQuery?: CustomQuery) => Promise<ProductsSearchResult<PRODUCT>>;
}

export function useProductFactory<PRODUCT, PRODUCT_SEARCH_PARAMS, API>(
  rawFactoryParams: UseProductFactoryParams<PRODUCT, PRODUCT_SEARCH_PARAMS, API>
) {
