...
Before setting up a new project, please add the `middlewareUrl` property, which define the URL to the Server Middleware. Usually, it's your application domain followed by the `/api` path. If you don't define this property
it will be automatically added to the application config on top of your application domain, followed by the `/api` path.
