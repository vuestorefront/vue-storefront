...
Before setting up a new project, please add the `middlewareUrl` property, which define the URL to the Server Middleware, is required to be defined. Usually, it's your application domain followed by the `/api` path. It's
also very important from the security perspective, that way your application will be not vulnerable to outside attacks.
