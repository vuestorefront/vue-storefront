...
----
To start working with Vue Storefront for {{ $themeLocaleConfig.label }} you have to install two core packages that will  integrate your {{ $themeLocaleConfig.label }} instance with Vue Storefront.

- [**{{ $themeLocaleConfig.label }} API Client**](./api-client) - A data layer of your application that connects directly to {{ $themeLocaleConfig.label }}. It provides a friendly abstraction layer over network calls with some additional overriding capabilities. It can be used as it is in any JavaScript application but doesn't provide much value by itself. API Client is usually used in the combination with Composition Functions package and is not used directly in the UI layer.
- **{{ $themeLocaleConfig.label }} Composition Functions** - A set of **declarative** composition functions allowing to interact with eCommerce logic in your Vue application that provides additional functionalities like Vue reactivity, SSR support, client-side request caching etc. This is the main integration package and it uses API Client under the hood.
...
Once packages are installed you need to invoke the `setup` method that will configure your {{ $themeLocaleConfig.label }} integration before using any other method from the integration. You can read how to configure it [here](./api-client).

<Content slot-key="setup" />

In the next chapters, you will learn in-depth about {{ $themeLocaleConfig.label }} API Client and Composition API functions.
