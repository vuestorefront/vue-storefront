...
import { extractSuggestionFromError, validateGitRepositoryURL } from "../../../src/domains/generate/git-repository-url";
...
describe("extractSuggestionFromError | unit tests", () => {
  it("extracts suggested git repository URL", async () => {
    ...
    (validateGitRepositoryURL as ValidateGitRepositoryURL).mockResolvedValueOnce(null);
  ...
  it("checks if suggestion is valid before returning it", async () => {
    (validateGitRepositoryURL as ValidateGitRepositoryURL).mockResolvedValueOnce(new Error("An expected error"));
