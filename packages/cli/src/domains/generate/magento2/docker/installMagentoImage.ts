...
import { logSimpleErrorMessage, logSimpleInfoMessage } from '../functions/terminalHelpers';
import removeDockerContainer from './removeDocker';

/** Handles Magento 2 Docker Image installation */
const installMagentoImage = async (magentoDirName: string, magentoDomainName: string, writeLog: (message: string) => void): Promise<any> => {
  const options = {
    cwd: magentoDirName,
  };
  ...
  return new Promise((resolve) => {
    ...
    sp.start(picocolors.cyan(t('command.generate_store.progress.docker_start')));

    curl.stdout.pipe(bash.stdin);

    bash.stdout.on('data', (data) => {
      if (data.toString().toLowerCase().includes('system') && data.toString().toLowerCase().includes('password')) {
        sp.stop(picocolors.yellow(t('command.generate_store.magento.password')));
      }
    ...
    bash.on('exit', async (code) => {
      if (code === 0) {
        sp.stop(picocolors.green(t('command.generate_store.progress.docker_end')));
        resolve(1);
      } else {
        sp.stop(picocolors.red(t('command.generate_store.progress.docker_failed')));

        if (stdout.includes('Project directory "/var/www/html/." is not empty')) {
