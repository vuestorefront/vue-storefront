...
const copyAuth = async (magentoDirName: string, accessKey: string, secretKey: string) => {
  ...
  const authFile = await fs.readFileSync(path.join(magentoDirName, "src/auth.json.sample"), "utf-8");

  await fs.writeFileSync(
    path.join(magentoDirName, "src/auth.json"),
    authFile.replace(/<public-key>/g, accessKey).replace(/<private-key>/g, secretKey),
    "utf-8"
  );
