...
- [x] 1.1 Add a client-side coupon activation module and result type that compose the existing cart getters, pending-coupon mutation, `cart/applyCoupon` action, cart-operation guard, and existing localized notifications.
...
- [x] 2.1 Process the initial `coupon_code` value from Vue Router's current route query through the browser adapter from the coupon activation module's one-time router-ready callback.
