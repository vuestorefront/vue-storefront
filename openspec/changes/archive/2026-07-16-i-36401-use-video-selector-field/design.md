...
The configured image remains in the rendered markup as the initial and recovery surface. SSR and hydration render no video `src`. After mounting, the component uses the injected `WindowObject` and one `matchMedia` query derived from the shared breakpoint value to assign the applicable asset URL directly to the `src` of one native video. An `addEventListener('change', ...)` listener replaces that URL when the viewport crosses the breakpoint, while URL watchers keep Storyblok preview changes reactive. The listener is removed during component teardown. `background_video.asset` and `background_video.aspect_ratio` define the desktop media. `mobile_background_video.asset` and `mobile_background_video.aspect_ratio` define the mobile media when specified; otherwise mobile remains on its configured static image rather than implicitly using the desktop video. The applicable selector aspect ratio sizes the media container, with the fallback image positioned inside that box, so the video is not cropped by an unrelated image ratio.
...
Native `source` elements with `media` attributes were rejected because an already-selected video resource is not reliably replaced when the viewport later crosses a breakpoint. Rendering separate autoplaying videos and hiding one with CSS was rejected because both resources can load and play. Client selection begins only after `mounted`, so the server and hydration output remain deterministic and image-first.
...
- [Responsive variants may cause unnecessary media downloads, duplicate playback, or stale playback after resize] → Render one native video, assign its active `src` URL after mount through `matchMedia`, replace it on `change` events, remove the listener during teardown, and verify that only the applicable asset is requested and played.
