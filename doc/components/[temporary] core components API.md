...
`filters.chosen` - a set of filters that user has defined on Category page.  
...
`imgObj` - a computed property that defines an image (thumbnail) that will be shown on the page and its size.  
`all_custom_attributes` - this is a subset of *attributesByCode* list of attributes that the current product has.  
