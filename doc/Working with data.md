...
We basicaly have following data stores accessible in the browser (`/src/store/index.js`):
...
const actions = {
  ...
  placeOrder ({ commit }, order) {
    const ajv = new Ajv()
    const validate = ajv.compile(require('src/store/modules/order/order.schema.json'))
...
The convention is, that schemas are stored under `/src/store/modules/<module-name>/<model-name>.schema.json` - for example [Order schema](https://github.com/DivanteLtd/vue-storefront/blob/master/src/store/modules/order/order.schema.json).
...
Here you have a validation schema for order: https://github.com/DivanteLtd/vue-storefront/blob/master/src/store/modules/order/order.schema.json
