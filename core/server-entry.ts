...
function _ssrHydrateSubcomponents(
  components,
  store,
  router,
  resolve,
      ...
      AsyncDataLoader.flush(
        ...
        .then(r => {
          if (buildTimeConfig.ssr.useInitialStateFilter) {
            context.state = omit(
              store.state,
              store.state.config.ssr.initialStateFilter
            );
          } else {
            context.state = store.state;
          }
          if (!buildTimeConfig.server.dynamicConfigReload) {
            // if dynamic config reload then we're sending config along with the request
            context.state = omit(
              store.state,
              buildTimeConfig.ssr.useInitialStateFilter
                ? [...store.state.config.ssr.initialStateFilter, 'config']
                : ['config']
            );
          } else {
            const excludeFromConfig =
              buildTimeConfig.server.dynamicConfigExclude;
            const includeFromConfig =
              buildTimeConfig.server.dynamicConfigInclude;
            console.log(excludeFromConfig, includeFromConfig);
            if (includeFromConfig && includeFromConfig.length > 0) {
              context.state.config = pick(
                context.state.config,
                includeFromConfig
              );
            }
            if (excludeFromConfig && excludeFromConfig.length > 0) {
              context.state.config = omit(
                context.state.config,
                excludeFromConfig
              );
            }
    ...
    router.onReady(() => {
      ...
      Promise.all(
        matchedComponents.map((Component: any) => {
          const components = Component.mixins
            ? Array.from(Component.mixins)
            : [];
