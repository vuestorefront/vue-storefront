...
const invokeClientEntry = async () => {
  ...
  router.onReady(() => {
    router.beforeResolve((to, from, next) => {
      ...
      if (Vue.prototype.$ssrRequestContext)
        Vue.prototype.$ssrRequestContext.output.cacheTags = new Set<string>();
      ...
      Promise.all(
        matched.map((c: any) => {
          ...
          const components =
            c.mixins && config.ssr.executeMixedinAsyncData
              ? Array.from(c.mixins)
              : [];
  ...
  EventBus.$on('order/PROCESS_QUEUE', event => {
    if (typeof navigator !== 'undefined' && navigator.onLine) {
      ...
      const ordersCollection = new UniversalStorage(
  ...
  EventBus.$on('sync/PROCESS_QUEUE', data => {
    if (typeof navigator !== 'undefined' && navigator.onLine) {
      ...
      const usersCollection = new UniversalStorage(
        localForage.createInstance({
      ...
      const cartsCollection = new UniversalStorage(
        localForage.createInstance({
          name: (config.storeViews.commonCache ? '' : dbNamePrefix) + 'shop',
          storeName: 'carts',
      ...
      usersCollection.getItem('current-token', (err, currentToken) => {
        // TODO: if current token is null we should postpone the queue and force re-login - only if the task requires LOGIN!
        if (err) {
          Logger.error(err)();
        }
        cartsCollection.getItem('current-cart-token', (err, currentCartId) => {
          ...
          if (!currentCartId && store.state.cart.cartServerToken) {
          ...
          if (!currentToken && store.state.user.token) {
            // this is workaround; sometimes after page is loaded indexedb returns null despite the cart token is properly set
            ...
            .iterate(
              (task, id, iterationNumber) => {
                if (task && !task.transmited && !mutex[id]) {
                  ...
                  fetchQueue.push(() => {
                    return execute(task, currentToken, currentCartId)
                      .then(executedTask => {
