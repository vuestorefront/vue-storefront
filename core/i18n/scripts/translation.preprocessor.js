...
module.exports = function(csvDirectories, config = null) {
  ...
  languages.forEach(language => {
    if (
      ...
      (config.i18n.bundleAllStoreviewLanguages && language === 'en-US')
    ) {
      console.debug(`Writing JSON file: ${language}.json`);
      fs.writeFileSync(
        path.join(__dirname, '../resource/i18n', `${language}.json`),
  ...
  if (config && config.i18n.bundleAllStoreviewLanguages) {
    ...
    bundledLanguages[config.i18n.defaultLocale] =
      messages[config.i18n.defaultLocale]; // default locale
    Object.keys(config.storeViews).forEach(storeCode => {
      ...
      if (store.hasOwnProperty('storeCode')) {
        if (!store.disabled && store.i18n) {
          bundledLanguages[store.i18n.defaultLocale] =
            messages[store.i18n.defaultLocale];
        }
      }
    });
    fs.writeFileSync(
