...
function build (overridingComponentsDir, overridingFilePath) {
  if (fs.existsSync('node_modules/@storefront-ui/vue/index.js')) {
    fs.readFile('node_modules/@storefront-ui/vue/index.js', 'utf8', (err, data) => {
      if (err) {
        return console.log(err);
      }

      // Replace relative links with absolute
      data = data.replace(new RegExp('./src', 'g'), '@storefront-ui/vue/src').replace(new RegExp('"', 'g'), "'")
      ...
      folders.forEach(folder => {
        files.forEach(file => {
          file = file.split('.')[0]
          data = data.replace('@storefront-ui/vue/src/components/' + folder + '/' + file + '/' + file + '.vue', '../../../src/themes/capybara/components/_overrides/' + file + '.vue')
        })
      })
