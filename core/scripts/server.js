...
function invalidateCache(req, res) {
  if (config.server.useOutputCache) {
    if (req.query.tag && req.query.key) {
      ...
      Promise.all(subPromises)
        .then(r => {
          apiStatus(
            res,
            `Tags invalidated successfully [${req.query.tag}]`,
            200
          );
        })
        .catch(error => {
    ...
    apiStatus(
      res,
      'Cache invalidation is not required, output cache is disabled',
      200
    );
  }
}
  ...
  const dynamicRequestHandler = renderer => {
    ...
    const context = {
      ...
      output: {
        prepend: context => {
          return '';
        }, // these functions can be replaced in the Vue components to append or prepend some content AFTER all other things are rendered. So in this function You may call: output.prepend() { return context.renderStyles() } to attach styles
        append: context => {
          return '';
        },
        appendHead: context => {
      ...
      cache
        ...
        .then(output => {
          if (output !== null) {
            ...
            res.end(output);
            console.log(
              `cache hit [${req.url}], cached request: ${Date.now() - s}ms`
            );
            next();
          } else {
            res.setHeader('Content-Type', 'text/html');
            ...
            console.log(
              `cache miss [${req.url}], request: ${Date.now() - s}ms`
            );
