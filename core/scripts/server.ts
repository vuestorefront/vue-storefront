...
interface ServerExtensions {
  ...
  configProvider?: (req: any) => Promise<any>
}
...
// TODO: This needs discussion. For now it's modified to have possibility to run a server
// const serverExtensions = require(resolve('src/modules/server'))
const serverExtensions: ServerExtensions = {
  serverModules: []
};
...
// TODO: This needs discussion. For now it's commented to have possibility to run a server
...
if (isProd) {
  ...
  const clientManifest = JSON.parse(readFileSync(resolve('dist/vue-ssr-client-manifest.json'), 'utf-8'));
  const bundle = JSON.parse(readFileSync(resolve('dist/vue-ssr-bundle.json'), 'utf-8'));
...
const serve = (path, cache, options?) => express.static(resolve(path), Object.assign({
  ...
  setHeaders: cache && isProd ? function (res, path) {
    const Mime = express.static.mime as unknown as {
      lookup: (path: string) => any,
      getType?: (path: string) => any
    };

    // TODO: There is something really weird with this dependency. Research needed.
    const mimeType = typeof Mime.getType === 'function'
      ? Mime.getType(path)
      : Mime.lookup(path);
...
const start = () => {
  app.listen(port, host)
    ...
    .on('error', (e) => {
      const parsedError = e as unknown as { code: string };
