...
class LocalForageCacheDriver {
  ...
  public constructor(
    ...
    if (isServer) {
      ...
      if (
        typeof Vue.prototype.$localCache[dbName][collectionName] === 'undefined'
      ) {
  ...
  public recreateDb() {
    if (this._localForageCollection._config) {
      const existingConfig = Object.assign(
        {},
        this._localForageCollection._config
      );
  ...
  public getItem(key, callback?) {
    ...
    if (this._useLocalCacheByDefault && this._localCache[key]) {
      ...
      return new Promise((resolve, reject) => {
        const value =
          typeof this._localCache[key] !== 'undefined'
    ...
    if (!isServer) {
      if (
        this.cacheErrorsCount[this._collectionName] >=
          DISABLE_PERSISTANCE_AFTER &&
        this._useLocalCacheByDefault
      ) {
        if (!this._persistenceErrorNotified) {
          Logger.error(
            'Persistent cache disabled becasue of previous errors [get]',
            key
          )();
          ...
          this._localForageCollection
            ...
            .then(result => {
              ...
              if (endTime - startTime >= CACHE_TIMEOUT) {
                Logger.error(
                  'Cache promise resolved after [ms]' +
                    key +
                    (endTime - startTime)
                )();
              }
              ...
              if (!isResolved) {
                ...
                Logger.debug(
                  'Skipping return value as it was previously resolved'
                )();
              }
              return result;
            })
  ...
  public setItem(key, value, callback?, memoryOnly = false) {
    ...
    if (!isServer) {
      if (
        this.cacheErrorsCount[this._collectionName] >=
          DISABLE_PERSISTANCE_AFTER_SAVE &&
        this._useLocalCacheByDefault
      ) {
        if (!this._persistenceErrorNotified) {
          Logger.error(
            'Persistent cache disabled becasue of previous errors [set]',
            key
          )();
          this._persistenceErrorNotified = true;
        }
