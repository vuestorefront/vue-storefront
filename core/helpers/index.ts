...
export function getThumbnailPath(relativeUrl, width, height) {
  if (rootStore.state.config.images.useExactUrlsNoProxy) {
    ...
    let baseUrl = rootStore.state.config.images.proxyUrl
      ? rootStore.state.config.images.proxyUrl
      : rootStore.state.config.images.baseUrl; // proxyUrl is not a url base path but contains {{url}} parameters and so on to use the relativeUrl as a template value and then do the image proxy opertions
    if (baseUrl.indexOf('{{') >= 0) {
      ...
      resultUrl = `${baseUrl}${parseInt(width)}/${parseInt(
        height
      )}/resize${relativeUrl}`;
    }
    return relativeUrl && relativeUrl.indexOf('no_selection') < 0
      ? resultUrl
      : rootStore.state.config.images.productPlaceholder || '';
  }
}
...
!isServer &&
  window.addEventListener('online', () => (onlineHelper.isOnline = true));
!isServer &&
  window.addEventListener('offline', () => (onlineHelper.isOnline = false));
