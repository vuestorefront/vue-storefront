...
const actions: ActionTree<CategoryState, RootState> = {
  ...
  async cacheProducts(
    ...
    const cachedProductsResponse = await dispatch(
      "product/list",
      {
        // configure and calculateTaxes is being executed in the product/list - we don't need another call in here
        query: filterQr,
  ...
  async configureProducts({ rootState }, { products = [], filters = {} } = {}) {
    return products.map(product => {
      ...
      return Object.assign(
        product,
        omit(configuredProductVariant, ["visibility"])
      );
    });
  },
  ...
  async loadCategories(
    ...
    if (!searchingByIds || categorySearchOptions.filters.id.length) {
      categorySearchOptions.filters = Object.assign(
        ...
        categorySearchOptions.filters
          ? cloneDeep(categorySearchOptions.filters)
          : {}
      );
      ...
      const notFoundCategories = searchedIds.filter(
        categoryId => !categories.some(cat => cat.id === parseInt(categoryId))
      );
  ...
  async loadAvailableFiltersFrom(
    ...
    commit(types.CATEGORY_SET_CATEGORY_FILTERS, {
      category,
      filters: resultFilters
    });
  },
  async switchSearchFilters(
    { dispatch },
  ...
  async loadCategoryBreadcrumbs(
    ...
    const categories = await dispatch("loadCategories", {
      ...
      reloadAll:
        Object.keys(config.entities.category.breadcrumbFilterFields).length > 0
    });
    const sorted = [];
    for (const id of categoryHierarchyIds) {
      ...
      if (
        index >= 0 &&
        (!omitCurrent || categories[index].id !== category.id)
      ) {
