  ...
  methods: {
    ...
    updateDetails() {
      ...
      if (!this.objectsEqual(this.shippingDetails, this.getShippingDetails())) {
        updatedShippingDetails = JSON.parse(
        ...
        if (this.currentUser.hasOwnProperty('default_shipping')) {
          ...
          for (let i = 0; i < this.currentUser.addresses.length; i++) {
            if (
              toString(this.currentUser.addresses[i].id) ===
              toString(this.currentUser.default_shipping)
            ) {
              index = i;
            }
          }
          if (index >= 0) {
            ...
            updatedShippingDetails.addresses[index].street = [
            ...
            updatedShippingDetails.addresses[index].telephone = this
          ...
          updatedShippingDetails.addresses.push({
            ...
            postcode: this.shippingDetails.postcode,
            ...(this.shippingDetails.phone
              ? { telephone: this.shippingDetails.phone }
              : {}),
    ...
    fillCompanyAddress() {
      ...
      if (this.useCompanyAddress) {
        ...
        for (let i = 0; i < this.currentUser.addresses.length; i++) {
          if (
            toString(this.currentUser.addresses[i].id) ===
            toString(this.currentUser.default_billing)
          ) {
            index = i;
          }
        }
        if (index >= 0) {
          this.shippingDetails.firstName = this.currentUser.addresses[
            index
          ].firstname;
          this.shippingDetails.lastName = this.currentUser.addresses[
            index
          ].lastname;
          this.shippingDetails.street = this.currentUser.addresses[
            index
          ].street[0];
          this.shippingDetails.house = this.currentUser.addresses[
            index
          ].street[1];
          this.shippingDetails.city = this.currentUser.addresses[index].city;
          this.shippingDetails.postcode = this.currentUser.addresses[
            index
          ].postcode;
          this.shippingDetails.region = this.currentUser.addresses[index].region
            .region
            ? this.currentUser.addresses[index].region.region
            : '';
          this.shippingDetails.country = this.currentUser.addresses[
            index
          ].country_id;
        }
      } else {
    ...
    getShippingDetails() {
      ...
      if (this.currentUser) {
        if (
          this.currentUser &&
          this.currentUser.hasOwnProperty('default_shipping')
        ) {
          let index;
          for (let i = 0; i < this.currentUser.addresses.length; i++) {
            if (
          ...
          if (index >= 0) {
            return {
              ...
              region: this.currentUser.addresses[index].region.region
                ? this.currentUser.addresses[index].region.region
                : '',
              country: this.currentUser.addresses[index].country_id,
              phone: this.currentUser.addresses[index].hasOwnProperty(
                'telephone'
              )
                ? this.currentUser.addresses[index].telephone
                : ''
            };
          }
        } else {
