      ...
      for (let i = 0; i < aProps.length; i++) {
        ...
        if (!b.hasOwnProperty(propName)) {
          ...
          if (
            ...
            !excludedFields.includes(propName) &&
      ...
      if (
        ...
        if (updatedProfile.hasOwnProperty('default_billing')) {
          ...
          for (let i = 0; i < updatedProfile.addresses.length; i++) {
            if (
              toString(updatedProfile.addresses[i].id) ===
              toString(updatedProfile.default_billing)
            ) {
              index = i;
            }
          }
          if (index >= 0) {
            if (this.addCompany) {
              updatedProfile.addresses[index].firstname =
                this.currentUser.firstname || '';
              updatedProfile.addresses[index].lastname =
                this.currentUser.lastname || '';
              updatedProfile.addresses[index].company =
                this.userCompany.company || '';
              updatedProfile.addresses[index].street = [
                this.userCompany.street,
                this.userCompany.house
              ] || ['', ''];
              updatedProfile.addresses[index].city =
                this.userCompany.city || '';
              updatedProfile.addresses[index].region = {
                region: this.userCompany.region ? this.userCompany.region : null
              };
              updatedProfile.addresses[index].country_id =
                this.userCompany.country || '';
              updatedProfile.addresses[index].postcode =
                this.userCompany.postcode || '';
              updatedProfile.addresses[index].vat_id =
                this.userCompany.taxId || '';
              updatedProfile.addresses[index].telephone =
          ...
          updatedProfile.addresses.push({
            ...
            ...(this.userCompany.region
              ? { region: { region: this.userCompany.region } }
              : {}),
    ...
    getUserCompany() {
      ...
      if (user.hasOwnProperty('default_billing')) {
        ...
        for (let i = 0; i < this.currentUser.addresses.length; i++) {
          if (
            toString(user.addresses[i].id) === toString(user.default_billing)
          ) {
            index = i;
          }
        }
        if (index >= 0) {
          return {
            ...
            region: user.addresses[index].region.region
              ? user.addresses[index].region.region
              : '',
