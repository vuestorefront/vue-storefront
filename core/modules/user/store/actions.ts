...
const actions: ActionTree<UserState, RootState> = {
  ...
  resetPassword(context, { email }) {
    return TaskQueue.execute({
      url: rootStore.state.config.users.resetPassword_endpoint,
      payload: {
        method: 'POST',
        mode: 'cors',
        headers: {
    ...
    return fetch(url, {
      ...
      .then(resp => {
        if (resp.code === 200) {
          ...
          context
            .dispatch('me', { refresh: true, useCache: false })
            .then(result => {});
          context
            .dispatch('getOrdersHistory', { refresh: true, useCache: false })
            .then(result => {});
        }
        return resp;
      });
  ...
  me(context, { refresh = true, useCache = true }) {
    return new Promise((resolve, reject) => {
      ...
      if (useCache === true) {
        // after login for example we shouldn't use cache to be sure we're loading currently logged in user
      ...
      if (refresh) {
        TaskQueue.execute({
          url: rootStore.state.config.users.me_endpoint,
          payload: {
            method: 'GET',
            mode: 'cors',
            headers: {
  ...
  changePassword(context, passwordData) {
    return TaskQueue.execute({
      url: rootStore.state.config.users.changePassword_endpoint,
      payload: {
        method: 'POST',
        mode: 'cors',
        headers: { 'Content-Type': 'application/json' },
  ...
  logout(context, { silent = false }) {
    ...
    context
      .dispatch('cart/serverTokenClear', {}, { root: true })
      .then(() => {
        context.dispatch('clearCurrentUser');
      })
      .then(() => {
        Vue.prototype.$bus.$emit('user-after-logout');
      })
      .then(() => {
  ...
  getOrdersHistory(context, { refresh = true, useCache = true }) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (useCache === true) {
        ...
        cache.getItem('orders-history', (err, res) => {
          ...
          if (res) {
            ...
            resolvedFromCache = true;
            Logger.log(
              'Current user order history served from cache',
              'user'
            )();
          }
        });
      ...
      if (refresh) {
        return TaskQueue.execute({
          url: rootStore.state.config.users.history_endpoint,
          payload: {
            method: 'GET',
            mode: 'cors',
            headers: {
  ...
  sessionAfterAuthorized(context, event) {
    ...
    rootStore
      .dispatch('user/me', { refresh: navigator.onLine }, { root: true })
      .then(us => {}); // this will load user cart
    rootStore
      .dispatch(
