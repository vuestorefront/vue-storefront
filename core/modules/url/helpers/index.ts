...
export function parametrizeRouteData(
  ...
  parametrizedRoute.params = Object.assign(
    {},
    parametrizedRoute.params || {},
    query
  );
  if (
    storeCodeInPath &&
    !parametrizedRoute.name.startsWith(storeCodeInPath + '-')
  ) {
...
export function processDynamicRoute(
  ...
  if (userRoute) {
    ...
    if (addToRoutes) {
      const routes = [];
      const rootDynamicRoute = Object.assign({}, userRoute, routeData, {
        path: '/' + fullPath,
        name: `urldispatcher-${fullPath}`
      });
      routes.push(rootDynamicRoute);
      if (
        config.storeViews.mapStoreUrlsFor.length > 0 &&
            ...
            const dynamicRoute = Object.assign({}, userRoute, routeData, {
              path:
                ...
                (rootStore.state.config.defaultStoreCode !== storeCode
                  ...
                  : '') +
                fullPath,
              name: `urldispatcher-${fullPath}-${storeCode}`
            });
...
export function formatCategoryLink(category: {
  ...
  return rootStore.state.config.seo.useUrlDispatcher
    ? '/' + category.url_path
    : (rootStore.state.config.products.useShortCatalogUrls ? '/' : '/c/') +
        category.slug;
}
