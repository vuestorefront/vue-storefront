...
export const UrlDispatchMapper = async to => {
  const routeData = await store.dispatch('url/mapUrl', {
    url: to.fullPath,
    query: to.query
  });
...
export function beforeEach(to: Route, from: Route, next) {
  if (isServer) {
    if (store.state.config.storeViews.multistore) {
      // this is called before server-entry.ts router.onReady - so we have to make sure we're in the right store context
  ...
  const hasRouteParams =
    to.hasOwnProperty('params') && Object.values(to.params).length > 0;
  ...
  if (
    ...
    UrlDispatchMapper(to)
      .then(routeData => {
        if (routeData) {
          let dynamicRoutes: LocalizedRoute[] = processDynamicRoute(
            routeData,
            fullPath,
          ...
          if (dynamicRoutes && dynamicRoutes.length > 0) {
            ...
            Logger.error(
              'Route not found ' + routeData['name'],
              'dispatcher'
            )();
            next('/page-not-found');
          }
      ...
      .catch(e => {
        ...
        if (!isServer) {
          ...
          Vue.prototype.$ssrRequestContext.server.response.redirect(
            (storeCode !== '' ? '/' + storeCode : '') + '/page-not-found'
          ); // TODO: Refactor this one after @filrak will give us a way to access ServerContext from Modules directly :-)
