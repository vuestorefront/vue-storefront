...
export const Shipping = {
  ...
  mounted() {
    if (
      ...
      this.notInMethods(this.shipping.shippingMethod)
    ) {
  ...
  methods: {
    ...
    onAfterPersonalDetails(receivedData) {
      if (!this.isFilled) {
        this.$store.dispatch('checkout/updatePropValue', [
          'firstName',
    ...
    useMyAddress() {
      if (this.shipToMyAddress) {
        this.shipping = {
          ...
          state: this.myAddressDetails.region.region
            ? this.myAddressDetails.region.region
            : '',
    ...
    getShippingMethod() {
      for (let i = 0; i < this.shippingMethods.length; i++) {
        if (
          this.shippingMethods[i].method_code === this.shipping.shippingMethod
        ) {
          return {
