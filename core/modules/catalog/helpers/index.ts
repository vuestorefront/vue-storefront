...
function _filterRootProductByStockitem(
  ...
  if (stockItem) {
    ...
    if (stockItem.is_in_stock === false) {
      ...
      Vue.prototype.$bus.$emit('product-after-removevariant', {
        product: product
      });
  ...
  if (rootStore.state.config.products.filterUnavailableVariants) {
    if (product.type_id === 'configurable' && product.configurable_children) {
      for (const stockItem of stockItems) {
        const confChild = product.configurable_children.find(p => {
          return p.id === stockItem.product_id;
        });
        if (
          stockItem.is_in_stock === false ||
      ...
      for (const optionKey in context.state.current_options) {
        ...
        if (optionsAvailable && optionsAvailable.length > 0) {
          optionsAvailable = optionsAvailable.filter(opt => {
            ...
            const variant = isOptionAvailableAsync(context, {
              product: product,
          ...
          Logger.debug(
            'Options still available' + optionsAvailable + removedOptions,
            'helper'
          )();
    ...
    if (rootStore.state.config.products.filterUnavailableVariants) {
      const _filterConfigurableHelper = () => {
        if (
          ...
          const stockItems = [];
          let confChildSkus = product.configurable_children.map(c => {
            return c.sku;
          });
          for (const confChild of product.configurable_children) {
            ...
            if (stockCached) {
              ...
              confChildSkus = remove(
                confChildSkus,
                skuToCheck => skuToCheck === confChild.sku
              );
            }
          }
          Logger.debug(
            'Cached stock items and delta' + stockItems + confChildSkus
          )();
          if (confChildSkus.length > 0) {
            context
              .dispatch('stock/list', { skus: confChildSkus }, { root: true })
              .then(task => {
                if (task && task.resultCode === 200) {
                  const diffLog = [];
                  _filterChildrenByStockitem(
                    context,
                    union(task.result, stockItems),
                    product,
                    diffLog
                  );
                  Logger.debug(
                    'Filtered configurable_children with the network call' +
                      diffLog,
                    'helper'
                  )();
                  resolve();
                  ...
                  Logger.error(
                    'Cannot sync the availability of the product options. Please update the vue-storefront-api or switch on the Internet',
                    'helper'
                  )();
                }
              })
            ...
            Logger.debug(
              'Filtered configurable_children without the network call' +
                diffLog,
              'helper'
            )();
            resolve();
      ...
      if (!rootStockCached) {
        context
          .dispatch('stock/list', { skus: [product.sku] }, { root: true })
          .then(task => {
            _filterRootProductByStockitem(
              context,
              task && task.result && task.result.length ? task.result[0] : null,
              product,
              reject
            );
...
export function syncProductPrice(product, backProduct) {
  ...
  product.special_price =
    backProduct.price_info.extension_attributes.tax_adjustments.special_price;
  product.price =
    backProduct.price_info.extension_attributes.tax_adjustments.final_price;
  product.originalPrice =
    backProduct.price_info.extension_attributes.tax_adjustments.regular_price;
  ...
  product.originalPriceTax =
    product.originalPriceInclTax - product.originalPrice;
  ...
  return new Promise((resolve, reject) => {
    if (rootStore.state.config.products.alwaysSyncPlatformPricesOver) {
        ...
        rootStore
          ...
          .then(syncResult => {
            if (syncResult) {
              ...
              for (let product of products) {
                const backProduct = syncResult.find(itm => {
                ...
                if (backProduct) {
                  ...
                  if (product.configurable_children) {
                    for (let configurableChild of product.configurable_children) {
                      const backProductChild = syncResult.find(itm => {
                        return itm.id === configurableChild.id;
                      });
                      if (backProductChild) {
                        configurableChild = syncProductPrice(
                          configurableChild,
                          backProductChild
                        );
                      }
      ...
      if (!rootStore.state.config.products.waitForPlatformSync && !isServer) {
        Logger.log(
          'Returning products, the prices yet to come from backend!'
        )();
        for (let product of products) {
...
export function calculateTaxes(products, store) {
  return new Promise((resolve, reject) => {
    if (rootStore.state.config.tax.calculateServerSide) {
      ...
      store.dispatch('tax/list', { query: '' }, { root: true }).then(tcs => {
        ...
        for (let product of products) {
          product = calculateProductTax(
            ...
            storeView.tax.defaultRegion,
            storeView.tax.sourcePriceIncludesTax
          );
        }
        doPlatformPricesSync(products).then(products => {
          resolve(products);
...
export function setBundleProductOptionsAsync(
  context,
...
export function configurableChildrenImages(product) {
  ...
  if (
    ...
    let configurableAttributes = product.configurable_options.map(
