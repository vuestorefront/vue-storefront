...
export const ProductBundleOption = {
  ...
  computed: {
    ...
    value() {
      return this.option.product_links.find(
        product => product.id === this.productOptionId
      );
    },
    errorMessage() {
  ...
  methods: {
    setDefaultValues() {
      if (this.option.product_links) {
        const defaultOption = this.option.product_links.find(pl => {
          return pl.is_default;
        });
        this.productOptionId = defaultOption
          ? defaultOption.id
          : this.option.product_links[0].id;
