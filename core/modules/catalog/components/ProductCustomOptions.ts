  ...
  methods: {
    ...
    setupInputFields() {
      for (let co of this.product.custom_options) {
        ...
        this.optionChanged(
          co,
          co.values && co.values.length > 0 ? co.values[0] : null
        );
      }
    },
    optionChanged(option, opval = null) {
      ...
      const value =
        opval === null ? this.inputValues[fieldName] : opval.option_type_id;
    ...
    validateField(option) {
      ...
      if (validationRule) {
        const validator = this.$store.state.product.custom_options_validators[
          validationRule
        ];
