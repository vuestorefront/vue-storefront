...
const actions: ActionTree<StockState, RootState> = {
  ...
  async check (context, { product, qty = 1 }) {
    if (rootStore.state.config.stock.synchronize) {
      const task:any = await TaskQueue.execute({ url: rootStore.state.config.stock.endpoint + '/check?sku=' + encodeURIComponent(product.sku),
        payload: {
          method: 'GET',
      ...
      const status = result.is_in_stock ? totalQty > result.qty ? 'not_enough_stock' : 'ok' : 'out_of_stock'
      return Object.assign({}, task, {qty: task.result.qty, status, onlineCheckTaskId: task.task_id})
    } else {
