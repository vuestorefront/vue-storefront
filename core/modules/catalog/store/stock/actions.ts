...
const actions: ActionTree<StockState, RootState> = {
   ...
   */
  queueCheck (context, { product, qty = 1 }) {
    ...
    return new Promise((resolve, reject) => {
      if (config.stock.synchronize) {
        TaskQueue.execute({ url: config.stock.endpoint + '/check?sku=' + encodeURIComponent(product.sku),
          ...
          resolve({ qty: task.result ? task.result.qty : 0, status: task.result ? (task.result.is_in_stock ? 'ok' : 'out_of_stock') : 'ok', onlineCheckTaskId: task.task_id }) // if online we can return ok because it will be verified anyway
        })
      } else {
        resolve({ qty: product.stock ? product.stock.qty : 0, status: product.stock ? (product.stock.is_in_stock ? 'ok' : 'out_of_stock') : 'volatile' }) // if not online, cannot check the source of true here
      }
  ...
  stockAfterCheck (context, event) {
    setTimeout(() => {
      ...
      context.dispatch('cart/getItem', event.product_sku, { root: true }).then((cartItem) => {
