...
const actions: ActionTree<CategoryState, RootState> = {
  ...
  list(
    context,
    {
      ...
      includeFields = rootStore.state.config.entities.optimize
        ? rootStore.state.config.entities.category.includeFields
        : null,
      excludeFields = rootStore.state.config.entities.optimize
        ? rootStore.state.config.entities.category.excludeFields
        : null,
      skipCache = false,
      updateState = true
    }
    ...
    if (level !== null) {
      ...
      if (
        level !==
          rootStore.state.config.entities.category
    ...
    if (
      ...
      return quickSearchByQuery({
        ...
        if (updateState) {
          commit(
            types.CATEGORY_UPD_CATEGORIES,
            Object.assign(resp, { includeFields, excludeFields })
          );
          Vue.prototype.$bus.$emit('category-after-list', {
            query: searchQuery,
            sort: sort,
            size: size,
            start: start,
      ...
      return new Promise((resolve, reject) => {
        let resp = {
          items: context.state.list,
          total: context.state.list.length
        };
        if (updateState) {
          Vue.prototype.$bus.$emit('category-after-list', {
            query: searchQuery,
    ...
    return new Promise((resolve, reject) => {
      const fetchCat = ({ key, value }) => {
        if (
          ...
          value >=
            rootStore.state.config.entities.category
              .categoriesRootCategorylId /* root category */
        ) {
          context
          ...
          reject(
            new Error(
              'Category query returned empty result ' + key + ' = ' + value
            )
          );
          return;
        }
      ...
      let setcat = (error, mainCategory) => {
        ...
        if (setCurrentCategoryPath) {
          ...
          let recurCatFinder = category => {
            ...
            if (
              category.parent_id >=
              rootStore.state.config.entities.category.categoriesRootCategorylId
            ) {
              dispatch('single', {
                key: 'id',
                ...
                .then(sc => {
                  ...
                  if (!sc) {
                    commit(
                      types.CATEGORY_UPD_CURRENT_CATEGORY_PATH,
                      currentPath
                    );
                    Vue.prototype.$bus.$emit('category-after-single', {
      ...
      if (state.list.length > 0 && !skipCache) {
        ...
        if (
          ...
          value >=
            rootStore.state.config.entities.category
              .categoriesRootCategorylId /** root category parent */
        ) {
  ...
  products(
    ...
    if (
      ...
      includeFields =
        rootStore.state.config.entities.productListWithChildren.includeFields; // we need configurable_children for filters to work
      excludeFields =
        rootStore.state.config.entities.productListWithChildren.excludeFields;
      ...
      if (rootStore.state.twoStageCachingDisabled) {
        Logger.log(
          'Two stage caching is disabled runtime because of no performance gain'
        )();
      } else {
        ...
        if (!res || res.noresults) {
          ...
          if (
            ...
            rootStore.state.config.products
              .configurableChildrenStockPrefetchStatic
          ) {
            ...
            res.items.map(i => {
              if (
                rootStore.state.config.products
                  .configurableChildrenStockPrefetchStaticPrefetchCount > 0
              ) {
                if (
                  prefetchIndex >
                  rootStore.state.config.products
              ...
              if (
                ...
                for (const confChild of i.configurable_children) {
                  const cachedItem =
                    context.rootState.stock.cache[confChild.id];
                  if (
                    typeof cachedItem === 'undefined' ||
                    cachedItem === null
                  ) {
            ...
            for (let attrToFilter of filters) {
              ...
              if (attrToFilter !== 'price') {
                if (res.aggregations['agg_terms_' + attrToFilter]) {
                  let buckets =
                    res.aggregations['agg_terms_' + attrToFilter].buckets;
                  if (
                    res.aggregations['agg_terms_' + attrToFilter + '_options']
                  ) {
                ...
                if (res.aggregations['agg_range_' + attrToFilter]) {
                  let index = 0;
                  let count =
                    res.aggregations['agg_range_' + attrToFilter].buckets
                      .length;
                  for (let option of res.aggregations[
                    'agg_range_' + attrToFilter
                  ].buckets) {
                    filterOptions.push({
                      id: option.key,
                      from: option.from,
                      ...
                      label:
                        index === 0 || index === count - 1
                          ...
                          : currencySign +
                            option.from +
                            (option.to ? ' - ' + option.to : '') // TODO: add better way for formatting, extract currency sign
                    });
                    index++;
                  }
                }
