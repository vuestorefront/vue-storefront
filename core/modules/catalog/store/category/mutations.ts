    ...
    for (let category of categories.items) {
      let catSlugSetter = category => {
        if (category.children_data) {
          for (let subcat of category.children_data) {
            // TODO: fixme and move slug setting to vue-storefront-api
            if (subcat.name) {
              subcat = Object.assign(subcat, {
                slug: subcat.slug
                  ? subcat.slug
                  : subcat.hasOwnProperty('url_key') &&
                    rootStore.state.config.products.useMagentoUrlKeys
                  ? subcat.url_key
                  : subcat.hasOwnProperty('name')
                  ? slugify(subcat.name) + '-' + subcat.id
                  : ''
              });
      ...
      if (categories.includeFields == null) {
        ...
        try {
          catCollection
            .setItem(
              entityKeyName('slug', category.slug.toLowerCase()),
              category
            )
            .catch(reason => {
