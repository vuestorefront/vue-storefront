...
const mutations: MutationTree<ProductState> = {
  ...
  [types.CATALOG_UPD_RELATED](state, { key, items }) {
    ...
    Vue.prototype.$bus.$emit('product-after-related', {
      key: key,
      items: items
    });
  },
  [types.CATALOG_ADD_CUSTOM_OPTION_VALIDATOR](
    state,
  ...
  [types.CATALOG_UPD_BUNDLE_OPTION](
    state,
  ...
  [types.CATALOG_UPD_PRODUCTS](state, { products, append }) {
    if (append === false) {
      ...
      (state.list as PagedProductList).items = (state.list as PagedProductList).items.concat(
        products.items
      );
    }
  },
