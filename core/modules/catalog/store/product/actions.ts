...
const actions: ActionTree<ProductState, RootState> = {
  ...
  async setupBreadcrumbs(context, { product }) {
    ...
    let setBreadcrumbRoutesFromPath = path => {
      if (
        path.findIndex(itm => {
          return (
            itm.slug === context.rootGetters['category/getCurrentCategory'].slug
          );
        }) < 0
      ) {
        path.push({
        ...
        .then(async categories => {
          ...
          for (let cat of catList) {
            ...
            if (
              ...
              catPath.split('/').length >
                catForBreadcrumbs.path.split('/').length
            ) {
  ...
  syncPlatformPricesOver(context, { skus }) {
    ...
    return TaskQueue.execute({
      url:
        rootStore.state.config.products.endpoint +
        '/render-list?skus=' +
    ...
    if (product.type_id === 'grouped') {
      ...
      if (product.product_links && product.product_links.length > 0) {
        for (let pl of product.product_links) {
          if (
            pl.link_type === 'associated' &&
            pl.linked_product_type === 'simple'
          ) {
            // prefetch links
            Logger.debug(
              'Prefetching grouped product link for ' +
                pl.sku +
                ' = ' +
                pl.linked_product_sku
            )();
            subloaders.push(
              context
                .dispatch('single', {
        ...
        Logger.error(
          'Product with type grouped has no product_links set!',
          product
        )();
      }
    }
      ...
      if (product.bundle_options && product.bundle_options.length > 0) {
        for (let bo of product.bundle_options) {
          ...
          for (let pl of bo.product_links) {
            Logger.debug(
              'Prefetching bundle product link for ' + bo.sku + ' = ' + pl.sku
            )();
            subloaders.push(
              context
                .dispatch('single', {
                  ...
                  if (asocProd) {
                    ...
                    if (pl.id === defaultOption.id) {
                      ...
                      product.priceInclTax +=
                        pl.product.priceInclTax * pl.product.qty;
          ...
          .then(attributes => {
            ...
            for (let option of product.configurable_options) {
              for (let ov of option.values) {
                ...
                if (trim(lb) !== '') {
                  let optionKey = option.attribute_code
    ...
    return quickSearchByQuery({
      ...
      return calculateTaxes(resp.items, context).then(updatedProducts => {
        ...
        for (let prod of resp.items) {
          ...
          if (
            (prod.type_id === 'grouped' || prod.type_id === 'bundle') &&
          ...
          .then(res => {
            if (res && res.items && res.items.length) {
              ...
              const _returnProductNoCacheHelper = subresults => {
                Vue.prototype.$bus.$emitFilter('product-after-single', {
                  key: key,
                  options: options,
                  product: prd
                });
              ...
              if (setCurrentProduct || selectDefaultVariant) {
                ...
                Promise.all(subConfigPromises).then(
        ...
        cache.getItem(cacheKey, (err, res) => {
          ...
          if (res !== null) {
            ...
            const _returnProductFromCacheHelper = subresults => {
              ...
              if (
                rootStore.state.config.products.alwaysSyncPlatformPricesOver
              ) {
  ...
  setCurrentErrors(context, errors) {
    if (errors && typeof errors === 'object') {
      context.commit(
        types.CATALOG_SET_PRODUCT_CURRENT,
        Object.assign({}, context.state.current, { errors: errors })
      );
    }
  },
  /**
    ...
    if (productVariant && typeof productVariant === 'object') {
      ...
      if (!rootStore.state.config.products.gallery.mergeConfigurableChildren) {
        context.commit(
          ...
          attributeImages(productVariant)
        );
      }
  ...
  fetch(context, { parentSku, childSku = null }) {
    ...
    return context
      .dispatch('single', { options: productSingleOptions })
      .then(product => {
        if (product.status >= 2) {
          throw new Error(
            `Product query returned empty result product status = ${
              product.status
            }`
          );
        }
        if (product.visibility === 1) {
          ...
          throw new Error(
            `Product query returned empty result product visibility = ${
              product.visibility
            }`
          );
        }
        ...
        if (product) {
          const productFields = Object.keys(product).filter(fieldName => {
            return (
              rootStore.state.config.entities.product.standardSystemFields.indexOf(
                fieldName
              ) < 0
          ...
          const attributesPromise = context.dispatch(
            'attribute/list',
            {
              ...
              filterValues: rootStore.state.config.entities.product
            ...
            { root: true }
          ); // TODO: it might be refactored to kind of: `await context.dispatch('attributes/list) - or using new Promise() .. to wait for attributes to be loaded before executing the next action. However it may decrease the performance - so for now we're just waiting with the breadcrumbs
          if (isServer) {
            subloaders.push(
              context.dispatch('setupBreadcrumbs', { product: product })
            );
            subloaders.push(
              context.dispatch('filterUnavailableVariants', {
                product: product
              })
            );
          } else {
            attributesPromise.then(() =>
              context.dispatch('setupBreadcrumbs', { product: product })
            ); // if this is client's side request postpone breadcrumbs setup till attributes are loaded to avoid too-early breadcrumb switch #2469
          ...
          if (
            rootStore.state.config.products
              .preventConfigurableChildrenDirectAccess
          ) {
            subloaders.push(
              context.dispatch('checkConfigurableParent', { product: product })
            );
          }
        } else {
          // error or redirect
  ...
  addCustomOptionValidator(context, { validationRule, validatorFunction }) {
    context.commit(types.CATALOG_ADD_CUSTOM_OPTION_VALIDATOR, {
      validationRule,
      validatorFunction
    });
  },
  ...
  setProductGallery(context, { product }) {
    if (
      ...
      if (
        !rootStore.state.config.products.gallery.mergeConfigurableChildren &&
        product.is_configured
      ) {
        context.commit(
          types.CATALOG_UPD_GALLERY,
          attributeImages(context.state.current)
        );
      } else {
        let productGallery = uniqBy(
          configurableChildrenImages(product).concat(getMediaGallery(product)),
          'src'
        ).filter(f => {
          return (
            f.src && f.src !== rootStore.state.config.images.productPlaceholder
          );
        });
  ...
  fetchAsync(context, { parentSku, childSku = null, route = null }) {
    if (
      context.state.productLoadStart &&
      Date.now() - context.state.productLoadStart < PRODUCT_REENTER_TIMEOUT
    ) {
      ...
      context.state.productLoadPromise = new Promise((resolve, reject) => {
        ...
        Logger.info('Fetching product data asynchronously', 'product', {
          parentSku,
          childSku
        })();
        Vue.prototype.$bus.$emit('product-before-load', {
          store: rootStore,
          route: route
        });
        context.dispatch('reset').then(() => {
          context
            .dispatch('fetch', { parentSku: parentSku, childSku: childSku })
            .then(subpromises => {
              Promise.all(subpromises)
                .then(subresults => {
                  Vue.prototype.$bus
                    .$emitFilter('product-after-load', {
                      store: rootStore,
                      route: route
                    })
                    .then(results => {
