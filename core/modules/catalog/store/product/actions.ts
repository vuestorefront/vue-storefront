...
const actions: ActionTree<ProductState, RootState> = {
  ...
  syncPlatformPricesOver ({ rootGetters }, { skus }) {
    ...
    let url = `${SideRequest(config.products, 'endpoint')}/render-list?skus=${encodeURIComponent(skus.join(','))}&currencyCode=${encodeURIComponent(storeView.i18n.currencyCode)}&storeId=${encodeURIComponent(storeView.storeId)}`
