  ...
  list(
    ...
    if (filterValues) {
      filterValues = filterValues.filter(fv => {
        ...
        if (
          context.state.blacklist !== null &&
          context.state.blacklist.includes(fv)
        )
          return false; // return that this attribute is not on our blacklist
        if (filterField === 'attribute_id')
          return (
            typeof context.state.list_by_id[fv] === 'undefined' ||
            context.state.list_by_id[fv] === null
          );
        if (filterField === 'attribute_code')
          return (
            typeof context.state.list_by_code[fv] === 'undefined' ||
            context.state.list_by_code[fv] === null
          );
      });
      if (!filterValues || filterValues.length === 0) {
        Logger.info(
          'Skipping attribute load - attributes already loaded',
          'attr',
          { orgFilterValues, filterField }
        )();
    ...
    return quickSearchByQuery({
      ...
      if (
        resp &&
        Array.isArray(orgFilterValues) &&
        orgFilterValues.length > 0
      ) {
        const foundValues = resp.items.map(attr => attr[filterField]);
        const toBlackList = filterValues.filter(
          ofv => !foundValues.includes(ofv)
        );
        toBlackList.map(tbl => {
          if (!context.state.blacklist.includes(tbl))
            context.state.blacklist.push(tbl);
