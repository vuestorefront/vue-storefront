...
const actions: ActionTree<CompareState, RootState> = {
  load({ commit, getters }, force: boolean = false) {
    ...
    cacheStorage.getItem('current-compare', (err, storedItems) => {
      ...
      Logger.info(
        'Compare state loaded from browser cache: ',
        'cache',
        storedItems
      )();
    });
  ...
  addItem({ commit }, product) {
    ...
    rootStore.dispatch('notification/spawnNotification', {
      type: 'success',
      message: i18n.t('Product {productName} has been added to the compare!', {
  ...
  removeItem({ commit }, product) {
    ...
    rootStore.dispatch('notification/spawnNotification', {
      type: 'success',
      message: i18n.t('Product {productName} has been removed from compare!', {
        productName: htmlDecode(product.name)
      }),
      action1: { label: i18n.t('OK') }
    });
