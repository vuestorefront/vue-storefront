...
function _afterServerItemUpdated(event, clientItem = null) {
  ...
  if (clientItem === null) {
    rootStore
      .dispatch('cart/getItem', event.result.sku, { root: true })
      .then(cartItem => {
        if (cartItem) {
...
const actions: ActionTree<CartState, RootState> = {
  ...
  clear(context) {
    ...
    if (config.cart.synchronize) {
      context.dispatch('serverCreate', {
        guestCart: !config.orders.directBackendSync
      }); // guest cart when not using directBackendSync because when the order hasn't been passed to Magento yet it will repopulate your cart
    }
  },
  save(context) {
    context.commit(types.CART_SAVE);
  },
  serverPull(context, { forceClientState = false, dryRun = false }) {
    // pull current cart FROM the server
    if (
      config.cart.synchronize &&
      !isServer &&
      onlineHelper.isOnline &&
      context.state.cartServerToken
    ) {
      const newItemsHash = sha3_224(
        JSON.stringify({
      ...
      if (
        Date.now() - context.state.cartServerPullAt >= CART_PULL_INTERVAL_MS ||
        newItemsHash !== context.state.cartItemsHash
      ) {
        ...
        return TaskQueue.execute({
          ...
          if (
            Date.now() - context.state.cartServerMethodsRefreshAt >=
            CART_METHODS_INTERVAL_MS
          ) {
            ...
            if (context.state.cartItems.length > 0) {
              let country = rootStore.state.checkout.shippingDetails.country
                ? rootStore.state.checkout.shippingDetails.country
                : storeView.tax.defaultCountry;
        ...
        Logger.log(
          'Too short interval for refreshing the cart or items not changed' +
            newItemsHash +
            context.state.cartItemsHash,
          'cart'
        )();
      }
    }
  ...
  serverTotals(context, { forceClientState = false }) {
    // pull current cart FROM the server
    if (
      config.cart.synchronize_totals &&
      !isServer &&
      onlineHelper.isOnline &&
      context.state.cartServerToken
    ) {
      if (
        Date.now() - context.state.cartServerTotalsAt >=
        CART_TOTALS_INTERVAL_MS
      ) {
        TaskQueue.execute({
          url: config.cart.totals_endpoint, // sync the cart
          payload: {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' },
            mode: 'cors'
        ...
        Logger.log(
          'Too short interval for refreshing the cart totals',
          'cart'
        )();
      }
    }
  ...
  serverCreate(context, { guestCart = false }) {
    if (config.cart.synchronize && !isServer) {
      if (
        Date.now() - context.state.cartServerCreatedAt >=
        CART_CREATE_INTERVAL_MS
      ) {
        const task = {
          url: guestCart
            ? config.cart.create_endpoint.replace('{{token}}', '')
            : config.cart.create_endpoint, // sync the cart
          payload: {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            mode: 'cors'
  ...
  serverUpdateItem(context, cartItem) {
    if (!cartItem.quoteId) {
      cartItem = Object.assign(cartItem, {
        quoteId: context.state.cartServerToken
      });
    }

    return TaskQueue.execute({
      url: config.cart.updateitem_endpoint, // sync the cart
      payload: {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        mode: 'cors',
  ...
  serverDeleteItem(context, cartItem) {
    ...
    return TaskQueue.execute({
      url: config.cart.deleteitem_endpoint, // sync the cart
      payload: {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        mode: 'cors',
  ...
  load(context) {
    return new Promise((resolve, reject) => {
      ...
      if (
        ...
        Array.isArray(context.rootGetters['shipping/shippingMethods'])
      ) {
        let shippingMethod = context.rootGetters[
          'shipping/shippingMethods'
        ].find(item => item.default);
        commit(types.CART_UPD_SHIPPING, shippingMethod);
      }
      if (
        (!state.payment || !state.payment.code) &&
        Array.isArray(context.rootGetters['payment/paymentMethods'])
      ) {
        let paymentMethod = context.rootGetters['payment/paymentMethods'].find(
          item => item.default
        );
        commit(types.CART_UPD_PAYMENT, paymentMethod);
      }
      Vue.prototype.$db.cartsCollection.getItem(
        'current-cart',
        (err, storedItems) => {
            ...
            Vue.prototype.$db.cartsCollection.getItem(
              ...
              (err, token) => {
                ...
                if (token) {
                  ...
                  Logger.info(
                    'Cart token received from cache.',
                    'cache',
                    token
                  )();
  ...
  addItem(
    ...
    { productToAdd, forceServerSilence = false }
  ) {
    let productsToAdd = [];
    if (productToAdd.type_id === 'grouped') {
      // TODO: add bundle support
      productsToAdd = productToAdd.product_links
        .filter(pl => {
          return pl.link_type === 'associated';
        })
        .map(pl => {
          return pl.product;
        });
    ...
    for (let product of productsToAdd) {
      ...
      if (product.qty && typeof product.qty !== 'number')
        product.qty = parseInt(product.qty);
      if (
        config.useZeroPriceProduct
      ...
      dispatch(
        ...
        if (productIndex === productsToAdd.length - 1 && productHasBeenAdded) {
          ...
          if (!config.externalCheckout) {
            // if there is externalCheckout enabled we don't offer action to go to checkout as it can generate cart desync
            notificationData.action2 = {
  ...
  getPaymentMethods(context) {
    if (
      ...
      TaskQueue.execute({
        url: config.cart.paymentmethods_endpoint,
        payload: {
          method: 'GET',
          headers: { 'Content-Type': 'application/json' },
          mode: 'cors'
        ...
        .then((task: any) => {
          ...
          let paymentMethods = context.rootGetters['payment/paymentMethods']
            .slice(0)
            .filter(itm => {
          ...
          for (let i = 0; i < backendMethods.length; i++) {
            if (
              typeof backendMethods[i] === 'object' &&
              !paymentMethods.find(item => item.code === backendMethods[i].code)
            ) {
          ...
          rootStore.dispatch('payment/replaceMethods', paymentMethods, {
            root: true
          });
          Vue.prototype.$bus.$emit(
            'set-unique-payment-methods',
  ...
  getShippingMethods(context, address) {
    return new Promise((resolve, reject) => {
      if (
        ...
        TaskQueue.execute({
          url: config.cart.shippingmethods_endpoint,
          payload: {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            mode: 'cors',
          ...
          .then((task: any) => {
            if (task.result.length > 0) {
              rootStore.dispatch('shipping/replaceMethods', task.result, {
                root: true
              });
              resolve(task.result);
            }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (
        ...
        if (methodsData.country && methodsData.carrier_code) {
          TaskQueue.execute({
            url: config.cart.shippinginfo_endpoint,
            payload: {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              mode: 'cors',
  ...
  removeCoupon(context) {
    return new Promise((resolve, reject) => {
      if (config.cart.synchronize_totals && onlineHelper.isOnline) {
        TaskQueue.execute({
          url: config.cart.deletecoupon_endpoint,
          payload: {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            mode: 'cors'
  ...
  applyCoupon(context, couponCode) {
    return new Promise((resolve, reject) => {
      if (config.cart.synchronize_totals && onlineHelper.isOnline) {
        TaskQueue.execute({
          url: config.cart.applycoupon_endpoint.replace(
            '{{coupon}}',
            couponCode
          ),
          payload: {
  ...
  userAfterLoggedin() {
    Vue.prototype.$db.usersCollection.getItem(
      'last-cart-bypass-ts',
      (err, lastCartBypassTs) => {
        if (err) {
          Logger.error(err, 'cart')();
        }
        if (
          !config.cart.bypassCartLoaderForAuthorizedUsers ||
          Date.now() - lastCartBypassTs >= 1000 * 60 * 24
        ) {
          // don't refresh the shopping cart id up to 24h after last order
          rootStore.dispatch(
            'cart/serverCreate',
            { guestCart: false },
            { root: true }
  ...
  servercartAfterCreated(context, event) {
    ...
    if (event.resultCode === 200) {
      ...
      rootStore.commit(
        types.SN_CART + '/' + types.CART_LOAD_CART_SERVER_TOKEN,
        cartToken
      );
      rootStore.dispatch(
        'cart/serverPull',
        { forceClientState: false, dryRun: !config.cart.serverMergeByDefault },
        { root: true }
      );
    } else {
      ...
      if (
        ...
        // not respond to unathorized errors here
        if (rootStore.state.cart.bypassCount < MAX_BYPASS_COUNT) {
          Logger.log(
            'Bypassing with guest cart' + rootStore.state.cart.bypassCount,
            'cart'
          )();
          rootStore.state.cart.bypassCount =
            rootStore.state.cart.bypassCount + 1;
          rootStore.dispatch(
            'cart/serverCreate',
            { guestCart: true },
            { root: true }
    ...
    if (event.resultCode === 200) {
      ...
      for (const clientItem of clientItems) {
        ...
        const serverItem = serverItems.find(itm => {
          return (
            itm.sku === clientItem.sku ||
            itm.sku.indexOf(clientItem.sku + '-') === 0
          ); /* bundle products */
        });

        if (!serverItem) {
          Logger.warn(
            'No server item with sku ' + clientItem.sku + ' on stock.',
            'cart'
          )();
          diffLog.push({
            party: 'server',
          ...
          if (!event.dry_run) {
            if (
              event.force_client_state ||
              !config.cart.serverSyncCanRemoveLocalItems
            ) {
              rootStore
                .dispatch(
                  'cart/serverUpdateItem',
                  {
                    sku:
          ...
          diffLog.push({
            ...
            sku: clientItem.sku,
            status: 'wrong_qty',
            client_qty: clientItem.qty,
            server_qty: serverItem.qty
          });
          if (!event.dry_run) {
            if (
              event.force_client_state ||
              !config.cart.serverSyncCanModifyLocalItems
            ) {
              rootStore
                .dispatch(
                  'cart/serverUpdateItem',
          ...
          Logger.info(
            'Server and client item with SKU ' +
              clientItem.sku +
              ' synced. Updating cart.',
            'cart',
            'cart'
          )();
      ...
      for (const serverItem of serverItems) {
        if (serverItem) {
          const clientItem = clientItems.find(itm => {
            return (
              itm.sku === serverItem.sku ||
              serverItem.sku.indexOf(itm.sku + '-') === 0
            ); /* bundle products */
          });
          if (!clientItem) {
            ...
            if (!event.dry_run) {
              if (event.force_client_state) {
                ...
                Logger.log(
                  'Removing item' + serverItem.sku + serverItem.item_id,
                  'cart'
                )();
                serverCartUpdateRequired = true;
                rootStore.dispatch(
                  'cart/serverDeleteItem',
                  {
                    sku: serverItem.sku,
                    item_id: serverItem.item_id,
                    quoteId: serverItem.quote_id
      ...
      if (!event.dry_run) {
        if (
          (!serverCartUpdateRequired || clientCartUpdateRequired) &&
          cartHasItems
        ) {
          rootStore.dispatch('cart/refreshTotals');
        }
      }
      Vue.prototype.$bus.$emit('servercart-after-diff', {
        diffLog: diffLog,
  ...
  servercartAfterItemUpdated(context, event) {
    ...
    if (event.resultCode !== 200) {
      ...
      if (originalCartItem.item_id) {
        rootStore
          .dispatch('cart/getItem', originalCartItem.sku, { root: true })
          .then(cartItem => {
            if (cartItem) {
              Logger.log(
                'Restoring qty after error' +
                  originalCartItem.sku +
                  cartItem.prev_qty,
                'cart'
              )();
              if (cartItem.prev_qty > 0) {
                rootStore.dispatch(
                  'cart/updateItem',
                  { product: { qty: cartItem.prev_qty } },
                  { root: true }
                ); // update the server_id reference
                Vue.prototype.$bus.$emit('cart-after-itemchanged', {
                  item: cartItem
                });
              } else {
                rootStore.dispatch(
                  'cart/removeItem',
                  { product: cartItem, removeByParentSku: false },
                  { root: true }
                ); // update the server_id reference
              }
            }
        ...
        rootStore.commit(
          'cart/' + types.CART_DEL_NON_CONFIRMED_ITEM,
          { product: originalCartItem },
          { root: true }
        );
      }
    } else {
