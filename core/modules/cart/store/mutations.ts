...
const mutations: MutationTree<CartState> = {
  ...
  [types.CART_DEL_NON_CONFIRMED_ITEM](
    state,
    { product, removeByParentSku = true }
  ) {
    Vue.prototype.$bus.$emit('cart-before-delete', { items: state.cartItems });
    state.cartItems = state.cartItems.filter(
      p =>
  ...
  [types.CART_UPD_TOTALS](
    state,
    { itemsAfterTotals, totals, platformTotalSegments }
  ) {
    ...
    Vue.prototype.$bus.$emit('cart-after-updatetotals', {
