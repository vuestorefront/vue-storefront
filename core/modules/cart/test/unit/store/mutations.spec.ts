...
describe('Cart mutations', () => {
  ...
  describe('CART_ADD_ITEM', () => {
    it('adds a product to cart if none of its sku is there yet', () => {
      ...
      const wrapper = (mutations: any) =>
        mutations[types.CART_ADD_ITEM](stateMock, { product });

      wrapper(cartMutations);

      expect(Vue.prototype.$bus.$emit).toBeCalledWith('cart-before-add', {
        product
      });
  ...
  it('CART_SAVE emits cart-before-save and updates save date', () => {
    ...
    expect(Vue.prototype.$bus.$emit).toBeCalledWith('cart-before-save', {
      items: stateMock.cartItems
    });
    expect(stateMock).toEqual(expectedState);
  });
  ...
  describe('CART_DEL_ITEM', () => {
    it('removes product from cart by sku', () => {
      ...
      expect(Vue.prototype.$bus.$emit).toBeCalledWith('cart-after-delete', {
        items: expectedState.cartItems
      });
      expect(stateMock).toEqual(expectedState);
    });
  ...
  describe('CART_UPD_ITEM', () => {
    it('updates product quantity by sku', () => {
      ...
      expect(Vue.prototype.$bus.$emit).toBeCalledWith('cart-before-update', {
        product: expectedState.cartItems[0]
      });
      expect(Vue.prototype.$bus.$emit).toBeCalledWith('cart-after-update', {
        product: expectedState.cartItems[0]
      });
      expect(stateMock).toEqual(expectedState);
    });
  ...
  describe('CART_UPD_ITEM_PROPS', () => {
    it('updates product properties by sku', () => {
      ...
      expect(Vue.prototype.$bus.$emit).toBeCalledWith(
  ...
  it('CART_LOAD_CART initializes cart with given products', () => {
    ...
    expect(Vue.prototype.$bus.$emit).toBeCalledWith('sync/PROCESS_QUEUE', {
      config: {}
    });
  ...
  it('CART_LOAD_CART initializes an empty cart when no products are given', () => {
    ...
    const wrapper = (mutations: any) =>
      mutations[types.CART_LOAD_CART](stateMock);
  ...
  it('CART_LOAD_CART_SERVER_TOKEN saves given cart token in cart data', () => {
    ...
    const wrapper = (mutations: any) =>
      mutations[types.CART_LOAD_CART_SERVER_TOKEN](
        stateMock,
  ...
  it('CART_TOGGLE_MICROCART changes microcart open status to the opposite one', () => {
    ...
    const wrapper = (mutations: any) =>
      mutations[types.CART_TOGGLE_MICROCART](stateMock);
