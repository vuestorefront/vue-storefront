...
export default {
  ...
  preAsyncData({ store, route }) {
    ...
    store.dispatch('category/setSearchOptions', {
      ...
      includeFields:
        store.state.config.entities.optimize && isServer
          ? store.state.config.entities.productList.includeFields
          : null,
      excludeFields:
        store.state.config.entities.optimize && isServer
          ? store.state.config.entities.productList.excludeFields
          : null,
      append: false
    });
  },
  async asyncData({ store, route, context }) {
    ...
    try {
      ...
      await store.dispatch('attribute/list', {
        ...
        includeFields:
          store.state.config.entities.optimize && isServer
            ? store.state.config.entities.attribute.includeFields
            : null
      });
      const parentCategory = await store.dispatch('category/single', {
        key: store.state.config.products.useMagentoUrlKeys ? 'url_key' : 'slug',
    ...
    validateRoute(route = this.$route) {
      ...
      this.$store
        .dispatch('category/single', {
          key: this.$store.state.config.products.useMagentoUrlKeys
            ? 'url_key'
            : 'slug',
          value: route.params.slug
        })
        .then(category => {
          if (!category) {
            ...
            let searchProductQuery = baseFilterProductsQuery(
              ...
              store.state.config.products.defaultFilters
            );
            this.$bus.$emit(
    ...
    return {
      ...
      meta: this.category.meta_description
        ? [
            {
              vmid: 'description',
              description: htmlDecode(this.category.meta_description)
            }
          ]
        : []
    };
