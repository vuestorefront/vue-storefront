...
export default {
  ...
  asyncData({ store, route, context }) {
    ...
    return store.dispatch('product/fetchAsync', {
  ...
  beforeMount() {
    ...
    this.$bus.$on(
      'product-after-customoptions',
      this.onAfterCustomOptionsChanged
    );
    this.$bus.$on(
      'product-after-bundleoptions',
      this.onAfterBundleOptionsChanged
    );
  ...
  methods: {
    ...
    onAfterCustomOptionsChanged(payload) {
      ...
      for (const optionValue of Object.values(payload.optionValues)) {
        if (
          ...
          parseInt(optionValue.option_type_id) > 0
        ) {
          ...
          if (optionValue.price_type === 'percent' && optionValue.price !== 0) {
            priceDelta +=
              (optionValue.price / 100) * this.originalProduct.price;
            priceDeltaInclTax +=
              (optionValue.price / 100) * this.originalProduct.priceInclTax;
          }
        }
      }
      this.product.price = this.originalProduct.price + priceDelta;
      this.product.priceInclTax =
        this.originalProduct.priceInclTax + priceDeltaInclTax;
    },
    onAfterBundleOptionsChanged(payload) {
      ...
      for (const optionValue of Object.values(payload.optionValues)) {
        if (
          typeof optionValue.value.product !== 'undefined' &&
          parseInt(optionValue.qty) >= 0
        ) {
          priceDelta +=
            optionValue.value.product.price * parseInt(optionValue.qty);
          priceDeltaInclTax +=
            optionValue.value.product.priceInclTax * parseInt(optionValue.qty);
        }
      }
      if (priceDelta > 0) {
    ...
    onStateCheck() {
      if (this.parentProduct && this.parentProduct.id !== this.product.id) {
        ...
        this.$router.replace({
          name: 'product',
          params: {
            parentSku: this.parentProduct.sku,
    ...
    onAfterPriceUpdate(product) {
      if (product.sku === this.product.sku) {
        ...
        this.$store
          .dispatch('product/setCurrent', omit(product, ['name']))
          .catch(err =>
            Logger.error({
    ...
    onUserPricesRefreshed() {
      if (this.$route.params.parentSku) {
        ...
        this.$bus.$emit('product-before-load', {
          store: this.$store,
          route: this.$route
        });
    ...
    return {
      ...
      meta: this.product.meta_description
        ? [
            {
              vmid: 'description',
              description: htmlDecode(this.product.meta_description)
            }
          ]
        : []
    };
