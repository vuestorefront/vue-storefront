...
const resetPassword = async (email: string): Promise<Task> =>
  TaskQueue.execute({
    url: processLocalizedURLAddress(SideRequest(config.users, 'resetPassword_endpoint')),
    payload: {
      method: 'POST',
      mode: 'cors',
      headers,
...
const login = async (username: string, password: string): Promise<Task> =>
  TaskQueue.execute({
    url: processLocalizedURLAddress(SideRequest(config.users, 'login_endpoint')),
    payload: {
      method: 'POST',
      mode: 'cors',
      headers,
...
const register = async (customer: DataResolver.Customer, password: string): Promise<Task> =>
  TaskQueue.execute({
    url: processLocalizedURLAddress(SideRequest(config.users, 'create_endpoint')),
    payload: {
      method: 'POST',
      headers,
...
const updateProfile = async (userProfile: UserProfile): Promise<Task> =>
  TaskQueue.execute({
    url: processLocalizedURLAddress(SideRequest(config.users, 'me_endpoint')),
    payload: {
      method: 'POST',
...
const getOrdersHistory = async (pageSize = 20, currentPage = 1): Promise<Task> =>
  TaskQueue.execute({
    url: processLocalizedURLAddress(
      SideRequest(config.users, 'history_endpoint').replace('{{pageSize}}', pageSize+'').replace('{{currentPage}}', currentPage+'')
    ),
    payload: {
      method: 'GET',
      mode: 'cors',
...
const changePassword = async (passwordData: DataResolver.PasswordData): Promise<Task> =>
  TaskQueue.execute({
    url: processLocalizedURLAddress(SideRequest(config.users, 'changePassword_endpoint')),
    payload: {
      method: 'POST',
      mode: 'cors',
      headers: { 'Content-Type': 'application/json' },
...
const refreshToken = async (refreshToken: string): Promise<string> =>
  fetch(processLocalizedURLAddress(SideRequest(config.users, 'refresh_endpoint')), {
    method: 'POST',
    mode: 'cors',
    headers,
