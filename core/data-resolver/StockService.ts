...
const queueCheck = (sku: string, actionName: string): Promise<any> =>
  TaskQueue.queue({
    url: processURLAddress(`${SideRequest(config.stock, 'endpoint')}/check?sku=${encodeURIComponent(sku)}`),
    payload: {
      method: 'GET',
...
const list = (skuList: string[]): Promise<Task> =>
  TaskQueue.execute({
    url: processURLAddress(
      `${SideRequest(config.stock, 'endpoint')}/list?skus=${encodeURIComponent(
        skuList.join(',')
      )}`
    ),
    payload: {
