...
export function prepareStoreView(storeCode: string): StoreView {
  ...
  const storeViewHasChanged =
    ...
    rootStore.state.storeView.storeCode !== storeCode;
  if (storeCode) {
    // current store code
...
export function storeCodeFromRoute(
  matchedRouteOrUrl: LocalizedRoute | RawLocation | string
): string {
  if (matchedRouteOrUrl) {
    for (const storeCode of rootStore.state.config.storeViews.mapStoreUrlsFor) {
      let urlPath =
        typeof matchedRouteOrUrl === 'object'
          ? matchedRouteOrUrl.path
          : matchedRouteOrUrl;
      if (urlPath.length > 0 && urlPath[0] !== '/') urlPath = '/' + urlPath;
      if (
        urlPath.startsWith('/' + storeCode + '/') ||
        urlPath === '/' + storeCode
      ) {
        return storeCode;
      }
    ...
    if (typeof routeObj === 'object') {
      ...
      if (routeObj.path) {
        routeObj.path =
          ...
          storeCode +
          '/' +
          (routeObj.path.startsWith('/')
            ? routeObj.path.slice(1)
            : routeObj.path);
      }
    } else {
    ...
    for (let storeCode of config.storeViews.mapStoreUrlsFor) {
      if (storeCode) {
        ...
        for (let route of routes) {
          const localRoute = localizedRoute(
            Object.assign({}, route),
            storeCode
          );
