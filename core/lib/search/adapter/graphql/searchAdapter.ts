...
import { SideRequest, isServer } from '@vue-storefront/core/helpers'
...
export class SearchAdapter {
  ...
  */
  public async search (Request) {
    ...
    let urlGql = ''
    if (SideRequest(this.entities[Request.type], 'url')) {
      urlGql = SideRequest(this.entities[Request.type], 'url')
    } else {
      
      const serverProtocol = isServer ? SideRequest(config.server, 'protocol') : config.server.protocol
      const host = isServer ? SideRequest(config.graphql, 'host') : config.graphql.host
      const port = isServer ? SideRequest(config.graphql, 'port') : config.graphql.port
