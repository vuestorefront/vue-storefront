...
export class SearchAdapter {
  ...
  public async search(Request) {
    ...
    if (Request.searchQuery instanceof SearchQuery) {
      ElasticsearchQueryBody = await prepareElasticsearchQueryBody(
        Request.searchQuery
      );
      if (Request.searchQuery.getSearchText() !== '') {
        ElasticsearchQueryBody['min_score'] =
          rootStore.state.config.elasticsearch.min_score;
      }
    } else {
    ...
    return fetch(url, {
      method: rootStore.state.config.elasticsearch.queryMethod,
      mode: 'cors',
      headers: {
      ...
      body:
        rootStore.state.config.elasticsearch.queryMethod === 'POST'
          ? JSON.stringify(ElasticsearchQueryBody)
          : null
    }).then(resp => {
      return resp.json();
    });
  ...
  public handleResult(resp, type, start = 0, size = 50): SearchResponse {
    ...
    if (resp.hasOwnProperty('hits')) {
        ...
        items: map(resp.hits.hits, hit => {
          return Object.assign(hit._source, {
            ...
            slug: hit._source.slug
              ? hit._source.slug
              : hit._source.hasOwnProperty('url_key') &&
      ...
      if (resp.error) {
        ...
        throw new Error(
          "Unknown error with elasticsearch result in resultPorcessor for entity type '" +
            type +
            "'"
        );
      }
