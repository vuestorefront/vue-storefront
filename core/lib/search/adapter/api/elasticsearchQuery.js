...
export async function prepareElasticsearchQueryBody(searchQuery) {
  const bodybuilder = await import(
    /* webpackChunkName: "bodybuilder" */ 'bodybuilder'
  );
  ...
  if (appliedFilters.length > 0) {
      ...
      if (filter.scope === 'default') {
        if (
          rangeOperators.every(rangeOperator =>
            filter.value.hasOwnProperty(rangeOperator)
          )
        ) {
    ...
    let attrFilterBuilder = (filterQr, attrPostfix = '') => {
      appliedFilters.forEach(catalogfilter => {
        ...
        if (catalogfilter.scope === 'catalog' && valueKeys.length) {
          const isRange = valueKeys.filter(
            value => rangeOperators.indexOf(value) !== -1
          );
          if (isRange.length) {
            ...
            // process terms filters
            let newValue =
              catalogfilter.value[Object.keys(catalogfilter.value)[0]];
            ...
            if (attrPostfix === '') {
              filterQr = filterQr.andFilter(
                'terms',
                getMapping(catalogfilter.attribute),
                newValue
              );
            } else {
              filterQr = filterQr.andFilter(
                'terms',
                catalogfilter.attribute + attrPostfix,
                newValue
              );
            }
          }
    ...
    if (hasCatalogFilters) {
      query = query
        .orFilter('bool', b => attrFilterBuilder(b))
        .orFilter('bool', b =>
          attrFilterBuilder(b, optionsPrfeix).filter(
            'match',
            'type_id',
            'configurable'
          )
      ...
      if (attrToFilter.scope === 'catalog') {
        if (attrToFilter.field !== 'price') {
          ...
          query = query.aggregation(
            'terms',
            getMapping(attrToFilter.field),
            aggregationSize
          );
          query = query.aggregation(
            'terms',
            attrToFilter.field + optionsPrfeix,
            aggregationSize
          );
        } else {
  ...
  let getQueryBody = function(b) {
    let searchableAttributes = config.elasticsearch.hasOwnProperty(
