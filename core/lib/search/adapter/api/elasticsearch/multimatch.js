...
function getConfig(queryText) {
  ...
  if (config.elasticsearch.queryMethod === 'GET') {
    ...
    minimumShouldMatch = scoringConfig.hasOwnProperty('minimum_should_match')
      ? scoringConfig.minimum_should_match + '25'
      : '75%25';
  } else {
    minimumShouldMatch = scoringConfig.hasOwnProperty('minimum_should_match')
      ? scoringConfig.minimum_should_match
      : '75%';
  }
  ...
  let multiMatchConfig = {
    ...
    cutoff_frequency: scoringConfig.cutoff_frequency
      ? scoringConfig.cutoff_frequency
      : '0.01',
    max_expansions: scoringConfig.max_expansions
      ? scoringConfig.max_expansions
      : '3',
    prefix_length: scoringConfig.prefix_length
      ? scoringConfig.prefix_length
      : '1',
