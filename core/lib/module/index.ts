...
class VueStorefrontModule {
  ...
  private _extendModule(extendedConfig: VueStorefrontModuleConfig): void {
    ...
    Logger.info(
      'Module "' + key + '" has been succesfully extended.',
      'module'
    )();
  }
  ...
  public register(): VueStorefrontModuleConfig | void {
    if (!this._isRegistered) {
      ...
      if (areStoresUnique) {
        ...
        if (this._c.store)
          VueStorefrontModule._extendStore(
            rootStore,
            this._c.store.modules,
            this._c.store.plugin
          );
        if (this._c.router)
