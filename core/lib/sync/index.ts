...
function queue(task) {
  ...
  return new Promise((resolve, reject) => {
    tasksCollection
      .setItem(task.task_id.toString(), task, (err, resp) => {
        if (err) Logger.error(err, 'sync')();
        Vue.prototype.$bus.$emit('sync/PROCESS_QUEUE', {
...
function execute(task) {
  ...
  const usersCollection = new UniversalStorage(
    localForage.createInstance({
      name:
        (rootStore.state.config.storeViews.commonCache ? '' : dbNamePrefix) +
        'shop',
      storeName: 'user',
      driver:
        localForage[rootStore.state.config.localForage.defaultDrivers['user']]
    })
  );
  const cartsCollection = new UniversalStorage(
    localForage.createInstance({
      ...
      storeName: 'carts',
      driver:
        localForage[rootStore.state.config.localForage.defaultDrivers['carts']]
    })
  );
  return new Promise((resolve, reject) => {
    if (isServer) {
      ...
      usersCollection.getItem('current-token', (err, currentToken) => {
        ...
        cartsCollection.getItem('current-cart-token', (err, currentCartId) => {
          ...
          if (!currentCartId && rootStore.state.cart.cartServerToken) {
