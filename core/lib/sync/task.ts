...
function _internalExecute(
  ...
  if (currentToken !== null && rootStore.state.userTokenInvalidateLock > 0) {
    // invalidate lock set
    Logger.log(
      'Waiting for rootStore.state.userTokenInvalidateLock to release for ' +
        task.url,
      'sync'
    )();
    _sleep(1000).then(() => {
      Logger.log(
        'Another try for rootStore.state.userTokenInvalidateLock for ' +
          task.url,
        'sync'
      )();
    ...
    resolve({
      code: 401,
      message: i18n.t(
        'Error refreshing user token. User is not authorized to access the resource'
      )
    })();
    return;
  } else {
    if (rootStore.state.userTokenInvalidated) {
      Logger.log(
        'Using new user token' + rootStore.state.userTokenInvalidated,
        'sync'
      )();
    ...
    .then(jsonResponse => {
      if (jsonResponse) {
        if (parseInt(jsonResponse.code) !== 200) {
          let resultString = jsonResponse.result
            ? toString(jsonResponse.result)
            : null;
          if (
            resultString &&
                  ...
                  rootStore
                    ...
                    .then(resp => {
                      if (resp.code === 200) {
                        ...
                        Logger.info(
                          'User token refreshed successfully' + resp.result,
                          'sync'
                        )();
                      } else {
                        rootStore.state.userTokenInvalidateLock = -1;
                        ...
                        TaskQueue.clearNotTransmited();
                        Logger.error(
                          'Error refreshing user token' + resp.result,
                          'sync'
                        )();
                      }
                    })
                    .catch(excp => {
                      ...
                      TaskQueue.clearNotTransmited();
                      Logger.error(
                        'Error refreshing user token' + excp,
                        'sync'
                      )();
                    });
                }
...
export function initializeSyncTaskStorage() {
  ...
  Vue.prototype.$db.syncTaskCollection = new UniversalStorage(
    localForage.createInstance({
      name: dbNamePrefix + 'shop',
      storeName: 'syncTasks',
      driver:
        localForage[
          rootStore.state.config.localForage.defaultDrivers['syncTasks']
        ]
    })
  );
}
